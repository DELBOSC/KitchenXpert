...
export class WebhookError extends ApiError {
  ...
  constructor(message: string, webhookId?: string, deliveryId?: string, statusCode?: number) {
    const details = {
      ...(webhookId && { webhookId }),
  ...
  static deliveryFailed(webhookId: string, deliveryId: string, statusCode?: number): WebhookError {
