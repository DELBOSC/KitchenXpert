...
export class MonitoringError extends ApiError {
  ...
  constructor(message: string, service?: string, metric?: string) {
    const details = {
      ...(service && { service }),
      ...(metric && { metric }),
    };
  ...
  static serviceUnhealthy(serviceName: string, reason?: string): MonitoringError {
    return new MonitoringError(
      reason
        ? `Service '${serviceName}' is unhealthy: ${reason}`
  ...
  static metricCollectionFailed(metricName: string, reason?: string): MonitoringError {
    return new MonitoringError(
      reason
        ? `Failed to collect metric '${metricName}': ${reason}`
        : `Failed to collect metric '${metricName}'`,
      undefined,
      metricName
    );
  }
  ...
  static alertingFailed(alertName: string, channel: string): MonitoringError {
    const error = new MonitoringError(`Failed to send alert '${alertName}' to ${channel}`);
