 ...
 *
 * SINGLE SOURCE OF TRUTH (§15.8 Principe 3 / Q3.b). Lives in @kitchenxpert/common
 * so BOTH the producer (packages/scraper Strategies) AND the consumer
 * (packages/backend persistence — roadmap step d) share one contract. The
 * scraper's local schemas/unified-product.schema.ts is now a re-export shim.
 */
import { z } from 'zod';
