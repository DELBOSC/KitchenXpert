...
export interface Permission { id: string; name: string; resource: string; action: string; description?: string; }
export interface Role { id: string; name: string; description?: string; permissions?: Permission[]; userCount?: number; createdAt: string; }
...
export const createRole = createAsyncThunk<Role, { name: string; description?: string; permissionIds?: string[] }>(
...
export const updateRole = createAsyncThunk<Role, { id: string; updates: Partial<Role> }>('roles/updateRole', async ({ id, updates }, { rejectWithValue }) => {
  try {
...
export const setRolePermissions = createAsyncThunk<Role, { id: string; permissionIds: string[] }>(
  'roles/setPermissions', async ({ id, permissionIds }, { rejectWithValue }) => {
    try {
...
const rolesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchRoles.rejected, (state, action) => { state.isLoading = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
