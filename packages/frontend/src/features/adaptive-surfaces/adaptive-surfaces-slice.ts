...
export const analyzeSurfaces = createAsyncThunk<SurfaceRecommendation[], { kitchenId: string; style?: string }>(
...
export const generateColorPalette = createAsyncThunk<string[], { baseColor: string; style?: string }>(
...
const adaptiveSurfacesSlice = createSlice({
  ...
  initialState,
  reducers: {
    setSurfaces: (state, action: PayloadAction<Surface[]>) => { state.surfaces = action.payload; },
    addSurface: (state, action: PayloadAction<Surface>) => { state.surfaces.push(action.payload); },
    updateSurface: (state, action: PayloadAction<{ id: string; updates: Partial<Surface> }>) => {
      ...
      if (idx !== -1 && existing) {
        state.surfaces[idx] = { ...existing, ...action.payload.updates, id: existing.id } as Surface;
      }
    },
    ...
    clearRecommendations: (state) => { state.recommendations = []; },
    clearError: (state) => { state.error = null; },
  },
  extraReducers: (builder) => {
    builder
      .addCase(fetchMaterials.pending, (state) => { state.isLoading = true; state.error = null; })
      .addCase(fetchMaterials.fulfilled, (state, action) => { state.isLoading = false; state.materials = action.payload; })
      .addCase(fetchMaterials.rejected, (state, action) => { state.isLoading = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
      .addCase(analyzeSurfaces.pending, (state) => { state.isAnalyzing = true; })
      .addCase(analyzeSurfaces.fulfilled, (state, action) => { state.isAnalyzing = false; state.recommendations = action.payload; })
      .addCase(analyzeSurfaces.rejected, (state, action) => { state.isAnalyzing = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
...
export const {
  setSurfaces, addSurface, updateSurface, removeSurface, selectSurface,
  setColorPalette, togglePreviewMode, clearRecommendations, clearError,
} = adaptiveSurfacesSlice.actions;

export const selectSurfaces = (state: { adaptiveSurfaces: AdaptiveSurfacesState }) => state.adaptiveSurfaces.surfaces;
export const selectMaterials = (state: { adaptiveSurfaces: AdaptiveSurfacesState }) => state.adaptiveSurfaces.materials;
export const selectSelectedSurface = createSelector(
  [(state: { adaptiveSurfaces: AdaptiveSurfacesState }) => state.adaptiveSurfaces.surfaces,
   (state: { adaptiveSurfaces: AdaptiveSurfacesState }) => state.adaptiveSurfaces.selectedSurfaceId],
  (surfaces, selectedSurfaceId) => surfaces.find(s => s.id === selectedSurfaceId)
);
export const selectRecommendations = (state: { adaptiveSurfaces: AdaptiveSurfacesState }) => state.adaptiveSurfaces.recommendations;
export const selectIsAnalyzing = (state: { adaptiveSurfaces: AdaptiveSurfacesState }) => state.adaptiveSurfaces.isAnalyzing;
