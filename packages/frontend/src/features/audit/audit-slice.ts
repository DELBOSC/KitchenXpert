...
export const fetchUserAuditLogs = createAsyncThunk<AuditLog[], { userId: string; limit?: number }>(
  'audit/fetchUserLogs',
  async ({ userId, limit = 100 }, { rejectWithValue }) => {
    try {
...
>('audit/fetchResourceHistory', async ({ resource, resourceId }, { rejectWithValue }) => {
  try {
    const response = await fetch(`${API_URL}/audit/resource/${resource}/${resourceId}/history`, {
      credentials: 'include',
    });
    const data = (await response.json()) as { data: AuditLog[]; error?: string };
    if (!response.ok) {
...
export const fetchAuditStats = createAsyncThunk<
  { byAction: Record<string, number>; byResource: Record<string, number> },
...
>('audit/fetchStats', async ({ startDate, endDate } = {}, { rejectWithValue }) => {
  try {
    ...
    const response = await fetch(`${API_URL}/audit/stats?${params.toString()}`, {
      credentials: 'include',
    });
    const data = (await response.json()) as {
      data: { byAction: Record<string, number>; byResource: Record<string, number> };
      error?: string;
    };
    if (!response.ok) {
      throw new Error(data.error);
...
export const exportAuditLogs = createAsyncThunk<AuditLog[], AuditFilters>(
  'audit/export',
