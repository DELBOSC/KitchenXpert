...
const initialState: WebhooksState = {
  webhooks: [], currentWebhook: null, events: [], failedEvents: [],
  isLoading: false, error: null, stats: null,
};
...
export const createWebhook = createAsyncThunk<Webhook, Partial<Webhook>>('webhooks/create', async (webhookData, { rejectWithValue }) => {
  try {
...
export const updateWebhook = createAsyncThunk<Webhook, { id: string; updates: Partial<Webhook> }>('webhooks/update', async ({ id, updates }, { rejectWithValue }) => {
  try {
...
const webhooksSlice = createSlice({
  ...
  initialState,
  reducers: {
    clearCurrentWebhook: (state) => { state.currentWebhook = null; },
    clearError: (state) => { state.error = null; },
  },
  extraReducers: (builder) => {
    builder
      .addCase(fetchWebhooks.pending, (state) => { state.isLoading = true; state.error = null; })
      .addCase(fetchWebhooks.fulfilled, (state, action) => { state.isLoading = false; state.webhooks = action.payload; })
      .addCase(fetchWebhooks.rejected, (state, action) => { state.isLoading = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
      .addCase(fetchWebhookById.fulfilled, (state, action) => { state.currentWebhook = action.payload; })
...
export const selectCurrentWebhook = (state: { webhooks: WebhooksState }) => state.webhooks.currentWebhook;
export const selectWebhookEvents = (state: { webhooks: WebhooksState }) => state.webhooks.events;
export const selectWebhooksLoading = (state: { webhooks: WebhooksState }) => state.webhooks.isLoading;
