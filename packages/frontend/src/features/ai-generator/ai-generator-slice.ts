...
const initialState: AIGeneratorState = {
  ...
  error: null,
  presets: [
    { id: 'modern', name: 'Modern Minimalist', description: 'Clean lines and neutral colors', input: { style: 'modern', preferences: { colorScheme: 'neutral' } } },
    { id: 'traditional', name: 'Traditional Classic', description: 'Warm wood tones and classic design', input: { style: 'traditional', preferences: { colorScheme: 'warm' } } },
    { id: 'industrial', name: 'Industrial Loft', description: 'Raw materials and open concept', input: { style: 'industrial', preferences: { materials: ['metal', 'concrete'] } } },
    { id: 'scandinavian', name: 'Scandinavian', description: 'Light, airy, and functional', input: { style: 'scandinavian', preferences: { colorScheme: 'light' } } },
  ],
};

const API_URL = import.meta.env.VITE_API_URL || '/api/v1';
export const generateLayout = createAsyncThunk<GenerationRequest, GenerationInput>(
  'aiGenerator/generateLayout', async (input, { rejectWithValue }) => {
    try {
...
export const generateDesign = createAsyncThunk<GenerationRequest, GenerationInput>(
  'aiGenerator/generateDesign', async (input, { rejectWithValue }) => {
    try {
...
export const optimizeLayout = createAsyncThunk<GenerationRequest, { kitchenId: string; goals?: string[] }>(
...
export const getSuggestions = createAsyncThunk<GenerationOutput, { kitchenId: string; category?: string }>(
...
export const applyGeneration = createAsyncThunk<void, { kitchenId: string; generationId: string }>(
  'aiGenerator/apply', async ({ kitchenId, generationId }, { rejectWithValue }) => {
    try {
...
const aiGeneratorSlice = createSlice({
  ...
  reducers: {
    ...
    clearCurrentRequest: (state) => { state.currentRequest = null; state.progress = 0; },
  ...
  extraReducers: (builder) => {
    builder
      .addCase(generateLayout.pending, (state) => { state.isGenerating = true; state.progress = 0; state.error = null; })
      .addCase(generateLayout.fulfilled, (state, action) => {
        state.isGenerating = false; state.progress = 100; state.currentRequest = action.payload;
        state.history.unshift(action.payload);
      })
      .addCase(generateLayout.rejected, (state, action) => { state.isGenerating = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
      .addCase(generateDesign.pending, (state) => { state.isGenerating = true; state.progress = 0; })
      .addCase(generateDesign.fulfilled, (state, action) => {
        state.isGenerating = false; state.progress = 100; state.currentRequest = action.payload;
        state.history.unshift(action.payload);
      })
      .addCase(generateDesign.rejected, (state, action) => { state.isGenerating = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
      .addCase(optimizeLayout.pending, (state) => { state.isGenerating = true; })
      .addCase(optimizeLayout.fulfilled, (state, action) => {
        state.isGenerating = false; state.currentRequest = action.payload;
        state.history.unshift(action.payload);
      })
      .addCase(optimizeLayout.rejected, (state, action) => { state.isGenerating = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
      .addCase(fetchGenerationHistory.fulfilled, (state, action) => { state.history = action.payload; });
  },
});

export const { setProgress, clearCurrentRequest, clearError, selectHistoryItem } = aiGeneratorSlice.actions;

export const selectCurrentGeneration = (state: { aiGenerator: AIGeneratorState }) => state.aiGenerator.currentRequest;
export const selectGenerationHistory = (state: { aiGenerator: AIGeneratorState }) => state.aiGenerator.history;
export const selectIsGenerating = (state: { aiGenerator: AIGeneratorState }) => state.aiGenerator.isGenerating;
export const selectGenerationProgress = (state: { aiGenerator: AIGeneratorState }) => state.aiGenerator.progress;
export const selectPresets = (state: { aiGenerator: AIGeneratorState }) => state.aiGenerator.presets;
