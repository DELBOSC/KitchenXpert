...
export interface Project {
  ...
  collaborators?: { userId: string; role: string; user?: { email: string; firstName: string; lastName: string } }[];
  kitchens?: { id: string; name: string }[];
  createdAt: string;
  updatedAt: string;
}

export interface ProjectFilters { status?: string; search?: string; }
...
export const createProject = createAsyncThunk<Project, Partial<Project>>('project/createProject', async (projectData, { rejectWithValue }) => {
  try {
...
export const updateProject = createAsyncThunk<Project, { id: string; updates: Partial<Project> }>('project/updateProject', async ({ id, updates }, { rejectWithValue }) => {
  try {
...
export const updateProjectStatus = createAsyncThunk<Project, { id: string; status: string }>('project/updateStatus', async ({ id, status }, { rejectWithValue }) => {
  try {
...
const projectSlice = createSlice({
  ...
  initialState,
  reducers: {
    setFilters: (state, action: PayloadAction<ProjectFilters>) => { state.filters = action.payload; },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchProjects.fulfilled, (state, action) => {
        state.isLoading = false; state.projects = action.payload.data;
      ...
      .addCase(fetchProjects.rejected, (state, action) => { state.isLoading = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
      ...
      .addCase(fetchProjectById.rejected, (state, action) => { state.isLoading = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
