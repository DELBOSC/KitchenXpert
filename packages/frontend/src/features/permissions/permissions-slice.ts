...
export interface Permission { id: string; name: string; resource: string; action: string; description?: string; createdAt: string; }
export interface GroupedPermissions { [resource: string]: Permission[]; }
...
const initialState: PermissionsState = { permissions: [], groupedPermissions: {}, resources: [], actions: [], isLoading: false, error: null };
...
export const createPermission = createAsyncThunk<Permission, { name: string; resource: string; action: string; description?: string }>(
...
const permissionsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchPermissions.fulfilled, (state, action) => { state.isLoading = false; state.permissions = action.payload; })
      .addCase(fetchPermissions.rejected, (state, action) => { state.isLoading = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
      .addCase(fetchGroupedPermissions.fulfilled, (state, action) => { state.groupedPermissions = action.payload; })
      .addCase(fetchResources.fulfilled, (state, action) => { state.resources = action.payload; })
      ...
      .addCase(deletePermission.fulfilled, (state, action) => { state.permissions = state.permissions.filter(p => p.id !== action.payload); });
  },
});
...
export const selectGroupedPermissions = (state: { permissions: PermissionsState }) => state.permissions.groupedPermissions;
