...
export const saveVRScene = createAsyncThunk<VRScene, { kitchenId: string; sceneData: Partial<VRScene> }>(
...
const vrSlice = createSlice({
  ...
  initialState,
  reducers: {
    enterVRMode: (state) => { state.isVRMode = true; state.viewMode = 'vr'; },
    exitVRMode: (state) => { state.isVRMode = false; state.viewMode = '3d'; },
    setViewMode: (state, action: PayloadAction<'3d' | 'vr' | 'ar'>) => { state.viewMode = action.payload; },
    updateSettings: (state, action: PayloadAction<Partial<VRSettings>>) => { state.settings = { ...state.settings, ...action.payload }; },
    resetSettings: (state) => { state.settings = defaultSettings; },
    selectObject: (state, action: PayloadAction<string | null>) => { state.selectedObjectId = action.payload; },
  ...
  extraReducers: (builder) => {
    builder
      .addCase(loadVRScene.pending, (state) => { state.isLoading = true; state.error = null; })
      .addCase(loadVRScene.fulfilled, (state, action) => { state.isLoading = false; state.currentScene = action.payload; })
      .addCase(loadVRScene.rejected, (state, action) => { state.isLoading = false; state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; })
      .addCase(saveVRScene.fulfilled, (state, action) => { state.currentScene = action.payload; })
      .addCase(saveVRScene.rejected, (state, action) => { state.error = (action.payload as string) ?? action.error?.message ?? 'An unknown error occurred'; });
  },
});

export const {
  enterVRMode, exitVRMode, setViewMode, updateSettings, resetSettings, selectObject,
  startWalkthrough, stopWalkthrough, updateCameraPosition, updateCameraTarget,
  setLightingPreset, clearError, clearScene,
} = vrSlice.actions;
