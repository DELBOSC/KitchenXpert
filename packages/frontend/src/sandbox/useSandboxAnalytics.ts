...
export type SandboxEvent =
  ...
  | {
      type: 'sandbox_first_action';
      props: { action: 'add_item' | 'move_item' | 'change_layout' | 'open_catalog' };
    }
  | {
      type: 'sandbox_friction_hit';
      props: {
        trigger: 'pdf_export' | 'ai_use' | 'quote_compare' | 'pathtracer' | 'session_15min';
      };
    }
  | {
      type: 'sandbox_signup_intent';
      props: {
        from: 'banner' | 'modal' | 'menu' | 'friction';
        trigger?: 'pdf_export' | 'ai_use' | 'quote_compare' | 'pathtracer' | 'session_15min';
      };
    }
  ...
  if (typeof window !== 'undefined' && typeof window.plausible === 'function') {
    try {
      window.plausible(event.type, {
        props: ('props' in event ? event.props : undefined) as
