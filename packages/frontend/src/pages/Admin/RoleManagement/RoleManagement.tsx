...
const RoleManagement: React.FC = () => {
  ...
  const handleCreateRole = async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : t('admin.roles.createFailed', 'Echec de la creation du role');
      setError(errorMessage);
    } finally {
      setIsSaving(false);
    }
  ...
  const handleUpdateRole = async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : t('admin.roles.updateFailed', 'Echec de la mise a jour du role');
      setError(errorMessage);
    } finally {
      setIsSaving(false);
    }
  ...
  const handleDeleteRole = (role: Role): void => {
    if (role.isSystem) {
      setError(
        t('admin.roles.systemCannotDelete', 'Les roles systeme ne peuvent pas etre supprimes')
      );
      return;
    }
  ...
  const confirmDeleteRole = async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : t('admin.roles.deleteFailed', 'Echec de la suppression du role');
      setError(errorMessage);
    } finally {
  ...
  const togglePermission = (permissionId: string, target: 'new' | 'edit'): void => {
    if (target === 'new') {
      setNewRole((prev) => ({
        ...prev,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="mt-1 text-gray-500 dark:text-gray-400">
              {t('admin.roleManagementDesc', 'Define roles and manage permissions')}
            </p>
          </div>
          <button
              ...
              <div className="p-6">
                ...
                <div className="mb-4">
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                    {t('admin.roles.permissionsAssigned', {
                      count: role.permissions.length,
                      defaultValue: '{{count}} permission(s) attribuee(s)',
                    })}
                  </p>
                  <div className="flex flex-wrap gap-1">
                    ...
                    {role.permissions.length > 5 && (
                      <span className="px-2 py-0.5 text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded">
                        {t('admin.roles.morePermissions', {
                          count: role.permissions.length - 5,
                          defaultValue: '+{{count}} de plus',
                        })}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-6 text-sm text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-gray-700 pt-4">
                  <span>
                    {t('admin.roles.userCount', {
                      ...
                      defaultValue: '{{count}} utilisateur(s)',
                    })}
                  </span>
                  <span>
                    {t('admin.roles.updatedAt', {
                      date: formatDate(role.updatedAt),
                      defaultValue: 'Mis a jour le {{date}}',
                    })}
                  </span>
                </div>
              </div>
        ...
        <div className="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
            {t('admin.roles.availablePermissions', 'Permissions disponibles')}
          </h2>
              ...
              <div key={category}>
                ...
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  {perms.map((permission) => (
                    <div key={permission.id} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
        ...
        >
          ...
          >
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              ...
              >
                {t('admin.roles.createRole', 'Creer un role')}
              </h2>
            </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                >
                  {t('admin.roles.nameLabel', 'Nom du role')}{' '}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t(
                    ...
                    'Decrire les responsabilites du role...'
                  )}
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                  {t('admin.roles.permissionsSelected', {
                    count: newRole.permissions.length,
                    defaultValue: '{{count}} permission(s) selectionnee(s)',
                  })}
                </p>
              </div>
            </div>
          ...
          >
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              ...
              >
                {t('admin.roles.editTitle', {
                  name: editingRole.name,
                  defaultValue: 'Modifier le role : {{name}}',
                })}
              </h2>
            </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                {editingRole.isSystem && (
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {t(
                      'admin.roles.systemNameReadonly',
                      'Les noms des roles systeme ne peuvent pas etre modifies'
                    )}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                  {t('admin.roles.permissionsSelected', {
                    count: editingRole.permissions.length,
                    defaultValue: '{{count}} permission(s) selectionnee(s)',
                  })}
                </p>
              </div>
          ...
          >
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              {t('admin.roles.deleteConfirm', {
                ...
                defaultValue:
                  'Etes-vous sur de vouloir supprimer le role "{{name}}" ? Cette action est irreversible.',
              })}
            </p>
