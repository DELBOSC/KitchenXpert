...
const AuditLogs: React.FC = () => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : t('admin.audit.errors.unexpected', 'An unexpected error occurred');
  ...
  const validateDateRange = (start: string, end: string): void => {
    if (start && end && start > end) {
      setDateRangeError(
        t('admin.audit.dateRangeError', 'La date de debut doit etre anterieure a la date de fin')
      );
    } else {
  ...
  const handleExportLogs = async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : t('admin.audit.errors.exportFailed', 'Export failed');
      setError(errorMessage);
    } finally {
        ...
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="mt-1 text-gray-500 dark:text-gray-400">
              {t('admin.auditLogsDesc', 'View and analyze system activity logs')}
            </p>
          </div>
          <button
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg flex justify-between items-center">
            ...
            <button
              ...
              aria-label={t('common.dismissError', 'Dismiss error')}
        ...
        <div className="grid gap-4 md:grid-cols-4 mb-6">
          {severities.map((severity) => {
            ...
            return (
              <div
                key={severity.value}
                className="bg-white dark:bg-gray-800 rounded-lg shadow-sm dark:shadow-gray-700/30 p-4"
              >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d={getCategoryIcon(log.category)}
                          />
                        </svg>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {log.userName ? (
                        ...
                        <span className="text-sm text-gray-500 dark:text-gray-400">
                          {t('admin.audit.cat.system', 'Systeme')}
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          {logs.length === 0 && !isLoading && (
            <div className="text-center py-12">
              ...
              <p className="mt-4 text-gray-500 dark:text-gray-400">
                {t('admin.audit.noResults', 'Aucun log ne correspond a vos criteres')}
              </p>
            </div>
          )}
                ...
                <div className="flex gap-2">
                  ...
                  <span className="px-4 py-2 text-gray-700 dark:text-gray-300" aria-current="page">
                    {t('common.pageOf', {
                      ...
                      defaultValue: 'Page {{current}} sur {{total}}',
                    })}
                  </span>
                  <button
                    onClick={() => handlePageChange(currentPage + 1)}
          ...
          >
            <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-start justify-between">
              <div>
                ...
                >
                  {t('admin.audit.detailTitle', 'Detail du log')}
                </h2>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <span
                    className={`px-2 py-1 text-xs font-medium rounded-full ${getSeverityColor(selectedLog.severity)}`}
                  >
              ...
              {selectedLog.userId && (
                <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                  <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
                    {t('admin.audit.userInfo', 'Informations utilisateur')}
                  </h3>
                  <div className="grid gap-4 md:grid-cols-2">
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-400">
                        {t('admin.audit.name', 'Nom')}
                      </label>
                      <p className="text-gray-900 dark:text-gray-200">
                        {selectedLog.userName || t('common.na', 'N/D')}
                      </p>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-400">
                        {t('admin.audit.email', 'Email')}
                      </label>
                      <p className="text-gray-900 dark:text-gray-200">
                        {selectedLog.userEmail || t('common.na', 'N/D')}
                      </p>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-400">
                        {t('admin.audit.userId', 'ID utilisateur')}
                      </label>
                      <p className="text-gray-900 dark:text-gray-200 font-mono text-sm">
                        {selectedLog.userId}
                      </p>
                    </div>
                  </div>
              ...
              {selectedLog.resourceType && (
                <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                  <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
                    {t('admin.audit.resourceInfo', 'Informations ressource')}
                  </h3>
                  <div className="grid gap-4 md:grid-cols-2">
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-400">
                        {t('admin.audit.resourceType', 'Type de ressource')}
                      </label>
                      <p className="text-gray-900 dark:text-gray-200">{selectedLog.resourceType}</p>
                    </div>
                    <div>
                      <label className="block text-xs text-gray-500 dark:text-gray-400">
                        {t('admin.audit.resourceId', 'ID de ressource')}
                      </label>
                      <p className="text-gray-900 dark:text-gray-200 font-mono text-sm">
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
                  {t('admin.audit.requestInfo', 'Informations requete')}
                </h3>
                <div className="space-y-2">
                  <div>
                    <label className="block text-xs text-gray-500 dark:text-gray-400">
                      {t('admin.audit.ipAddress', 'Adresse IP')}
                    </label>
                    <p className="text-gray-900 dark:text-gray-200 font-mono text-sm">
                      {selectedLog.ipAddress || t('common.na', 'N/D')}
                    </p>
                  </div>
                  <div>
                    <label className="block text-xs text-gray-500 dark:text-gray-400">
                      {t('admin.audit.userAgent', 'Agent utilisateur')}
                    </label>
                    <p className="text-gray-900 dark:text-gray-200 text-sm break-all">
                      {selectedLog.userAgent || t('common.na', 'N/D')}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
                  {t('admin.audit.additionalDetails', 'Details supplementaires')}
                </h3>
