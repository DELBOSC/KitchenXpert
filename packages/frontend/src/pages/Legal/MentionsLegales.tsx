...
export default function MentionsLegales(): React.ReactElement {
  ...
  return (
    <LegalLayout title="Mentions légales">
      ...
      <p>
        Le directeur de la publication est <strong>{editor.directorOfPublication}</strong>,
        en qualité de Président de {editor.socialName}.
      </p>
      ...
      <p>
        ...
        consommation, et après une tentative de résolution préalable
        directement auprès de notre service client
        (<a href={`mailto:${editor.email}`}>{editor.email}</a>), le
      ...
      <p>
        ...
        Email&nbsp;:{' '}
        <a href={`mailto:${mediator.email}`}>{mediator.email}</a><br />
        Site&nbsp;:{' '}
...
function EditorBlock(): React.ReactElement {
  ...
  const capitalLine = editor.capitalEuros !== null
    ? ` au capital de ${editor.capitalEuros.toLocaleString('fr-FR')} €`
    : '';

  return (
    <p>
      <strong>{editor.socialName}</strong><br />
      {editor.legalForm}{capitalLine}<br />
      Siège social&nbsp;: {addressLines.join(' — ')}<br />
      SIREN&nbsp;: {editor.siren}<br />
      SIRET&nbsp;: {editor.siret}<br />
      RCS {editor.rcsCity} — n° {editor.siren}<br />
      N° TVA intracommunautaire&nbsp;: {editor.vatNumber}<br />
      Email&nbsp;:{' '}
      <a href={`mailto:${editor.email}`}>{editor.email}</a>
      {editor.phone ? (
        <>
          <br />Téléphone&nbsp;: {editor.phone}
        </>
      ) : null}
    </p>
  );
