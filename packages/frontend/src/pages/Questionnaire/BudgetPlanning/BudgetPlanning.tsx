...
const BudgetPlanning: React.FC = () => {
  ...
  const currencies = [
    ...
    { code: 'EUR', symbol: '\u20AC', name: t('questionnaire.budget.currencies.eur', 'Euro') },
    {
      code: 'GBP',
      symbol: '\u00A3',
      name: t('questionnaire.budget.currencies.gbp', 'British Pound'),
    },
    {
      code: 'CAD',
      symbol: 'C$',
      name: t('questionnaire.budget.currencies.cad', 'Canadian Dollar'),
    },
    {
      code: 'AUD',
      symbol: 'A$',
      name: t('questionnaire.budget.currencies.aud', 'Australian Dollar'),
    },
  ];
  ...
  const priorityOptions = [
    {
      id: 'storage',
      name: t('questionnaire.budget.priorities.storage', 'Maximum Storage'),
      icon: 'M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4',
    },
    {
      id: 'appliances',
      name: t('questionnaire.budget.priorities.appliances', 'Premium Appliances'),
      icon: 'M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z',
    },
    {
      id: 'aesthetics',
      name: t('questionnaire.budget.priorities.aesthetics', 'Visual Appeal'),
      icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z',
    },
    {
      id: 'workspace',
      name: t('questionnaire.budget.priorities.workspace', 'Counter Space'),
      icon: 'M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z',
    },
    {
      id: 'durability',
      name: t('questionnaire.budget.priorities.durability', 'Durability'),
      icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
    },
    {
      id: 'eco',
      name: t('questionnaire.budget.priorities.eco', 'Eco-Friendly'),
  ...
  const budgetRanges = [
    {
      min: 0,
      max: 15000,
      label: t('questionnaire.budget.ranges.budgetFriendly', 'Budget-Friendly'),
      description: t(
        'questionnaire.budget.ranges.budgetFriendlyDesc',
        'Essential updates with cost-effective materials'
      ),
    },
    {
      min: 15000,
      max: 35000,
      label: t('questionnaire.budget.ranges.midRange', 'Mid-Range'),
      description: t(
        'questionnaire.budget.ranges.midRangeDesc',
        'Quality materials and some premium features'
      ),
    },
    {
      min: 35000,
      max: 75000,
      label: t('questionnaire.budget.ranges.premium', 'Premium'),
      description: t(
        'questionnaire.budget.ranges.premiumDesc',
        'High-end materials and professional design'
      ),
    },
    {
      min: 75000,
      max: Infinity,
      label: t('questionnaire.budget.ranges.luxury', 'Luxury'),
      description: t(
        'questionnaire.budget.ranges.luxuryDesc',
        'Top-tier everything with custom features'
      ),
    },
  ];
  ...
  useEffect(() => {
    ...
    const fetchBudgetData = async (): Promise<void> => {
      try {
        ...
        logger.debug(
          'Failed to fetch budget data, using defaults',
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.totalBudget <= 0) {
      newErrors.totalBudget = t(
        'questionnaire.budget.errors.invalidBudget',
  ...
  const handleGenerateDesigns = async (): Promise<void> => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        throw new Error(
          result.error || t('common.unexpectedError', 'An unexpected error occurred')
        );
      }
    } catch (err) {
      const errorMessage =
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
              {t('questionnaire.step', { current: 4, total: 4, defaultValue: 'Step 4 of 4' })}
            </span>
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {t('questionnaire.budgetPlanning', 'Budget Planning')}
            </span>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm dark:shadow-gray-700/30 p-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
            {t('questionnaire.budget.title', 'Budget Planning')}
          </h1>
          <p className="text-gray-600 dark:text-gray-300 mb-6">
            {t(
              'questionnaire.budget.subtitle',
              'Define your budget and spending priorities for your kitchen renovation.'
            )}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <section>
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                {t('questionnaire.budget.totalBudget', 'Total Budget')}
              </h2>
              ...
              {currentRange && formData.totalBudget > 0 && (
                <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                  <p className="font-medium text-blue-900 dark:text-blue-300">
                    {t('questionnaire.budget.kitchenType', '{{range}} Kitchen', {
            ...
            <section>
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                {t('questionnaire.budget.flexibility', 'Budget Flexibility')}
              </h2>
                ...
                {(['strict', 'moderate', 'flexible'] as const).map((level) => (
                  ...
                  >
                    <p className="font-medium text-gray-900 dark:text-white capitalize">
                      {t(`questionnaire.budget.flexibilityLevel.${level}`, level)}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                      {level === 'strict' &&
                        t('questionnaire.budget.flexibilityDesc.strict', 'Cannot exceed budget')}
                      {level === 'moderate' &&
                        t(
                          'questionnaire.budget.flexibilityDesc.moderate',
                          'Up to 10-15% over if needed'
                        )}
                      {level === 'flexible' &&
                        t('questionnaire.budget.flexibilityDesc.flexible', 'Budget is a guideline')}
                    </p>
                  </button>
                ))}
              </div>
            ...
            <section>
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">
                {t('questionnaire.budget.priorityAreas', 'Priority Areas')}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                {t(
                  'questionnaire.budget.priorityAreasDesc',
                  'Select up to 3 priorities for your budget allocation'
                )}
              </p>
              ...
              <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                {priorityOptions.map((option) => (
                  ...
                  >
                    <svg
                      className={`w-6 h-6 mb-2 ${formData.priorityAreas.includes(option.id) ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
            ...
            <section>
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                {t('questionnaire.budget.budgetIncludes', 'Budget Includes')}
              </h2>

              <div className="space-y-3">
                {[
                  {
                    name: 'includeProfessionalFees',
                    label: t(
                      'questionnaire.budget.includes.professionalFees',
                      'Professional design & installation fees'
                    ),
                  },
                  {
                    name: 'includeAppliances',
                    label: t('questionnaire.budget.includes.appliances', 'New appliances'),
                  },
                  {
                    name: 'includeLighting',
                    label: t('questionnaire.budget.includes.lighting', 'Lighting fixtures'),
                  },
                  {
                    name: 'includeFlooring',
                    label: t('questionnaire.budget.includes.flooring', 'Flooring replacement'),
                  },
                ].map((item) => (
            ...
            <section>
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                {t('questionnaire.budget.breakdown', 'Budget Breakdown')}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                {t(
                  'questionnaire.budget.breakdownDesc',
                  'Adjust the percentage allocation for each category'
                )}
              </p>
              ...
              <div className="space-y-4">
                {[
                  {
                    key: 'cabinets',
                    label: t('questionnaire.budget.categories.cabinets', 'Cabinets & Storage'),
                  },
                  {
                    key: 'countertops',
                    label: t('questionnaire.budget.categories.countertops', 'Countertops'),
                  },
                  {
                    key: 'appliances',
                    label: t('questionnaire.budget.categories.appliances', 'Appliances'),
                  },
                  {
                    key: 'labor',
                    label: t('questionnaire.budget.categories.labor', 'Labor & Installation'),
                  },
                  {
                    key: 'other',
                    label: t(
                      'questionnaire.budget.categories.other',
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleBreakdownChange(
                            category.key as keyof BudgetData['breakdown'],
                ...
                {Object.values(formData.breakdown).reduce((a, b) => a + b, 0) !== 100 && (
                  <p className="text-sm text-yellow-600 dark:text-yellow-400 mt-2">
                    {t(
                      'questionnaire.budget.percentageWarning',
                      'Note: Percentages total {{total}}% (should equal 100%)',
                      { total: Object.values(formData.breakdown).reduce((a, b) => a + b, 0) }
                    )}
                  </p>
                )}
              </div>
            ...
            <section>
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                {t('questionnaire.budget.timeline', 'Project Timeline')}
              </h2>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {(
                  [
                    {
                      value: 'urgent',
                      label: t('questionnaire.budget.timelineOptions.urgent', 'Urgent'),
                      description: t(
                        'questionnaire.budget.timelineOptions.urgentDesc',
                        '1-2 months'
                      ),
                    },
                    {
                      value: 'standard',
                      label: t('questionnaire.budget.timelineOptions.standard', 'Standard'),
                      description: t(
                        'questionnaire.budget.timelineOptions.standardDesc',
                        '2-4 months'
                      ),
                    },
                    {
                      value: 'flexible',
                      label: t('questionnaire.budget.timelineOptions.flexible', 'Flexible'),
                      description: t(
                        'questionnaire.budget.timelineOptions.flexibleDesc',
                        '4+ months'
                      ),
                    },
                  ] as const
            ...
            <section>
              <label
                htmlFor="financingNeeded"
                aria-label={t(
                  'questionnaire.budget.financing',
                  "I'm interested in financing options"
                )}
                className="flex items-center gap-3 cursor-pointer p-4 bg-gray-50 dark:bg-gray-700 rounded-lg"
              >
                <input
                  id="financingNeeded"
                  type="checkbox"
                ...
                <span>
                  <span className="block font-medium text-gray-900 dark:text-white">
                    {t('questionnaire.budget.financing', "I'm interested in financing options")}
                  </span>
                  <span className="block text-sm text-gray-500 dark:text-gray-400">
                    {t(
                      'questionnaire.budget.financingDesc',
                      "We'll provide information about available financing plans"
                    )}
                  </span>
                </span>
              </label>
              ...
              <div className="mt-6">
                <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-3">
                  <div
                    ...
                    role="status"
                    aria-label={t('questionnaire.aiAnalysisLoading', 'AI analysis loading...')}
                  />
            ...
            {aiTips && (
              <div className="mt-6 space-y-3">
                {aiTips.budgetReality && (
                  ...
                  >
                    <p className="font-medium mb-1">
                      {aiTips.budgetReality.isRealistic
                        ? t('questionnaire.budget.aiRealistic', 'Realistic budget')
                        : t('questionnaire.budget.aiUnrealistic', 'Budget needs revision')}
                    </p>
                    <p>{aiTips.budgetReality.explanation}</p>
                    <p className="mt-1 text-xs">
                      {t(
                        'questionnaire.budget.suggestedRange',
                        'Suggested range: {{min}} - {{max}}',
                        {
                          min: aiTips.budgetReality.suggestedRange.min.toLocaleString(
                            i18n.language
                          ),
                          max: aiTips.budgetReality.suggestedRange.max.toLocaleString(
                            i18n.language
                          ),
                        }
                      )}
                ...
                {aiTips.tips.map((tip, i) => (
                  <div
                    key={i}
                    className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-3 text-sm text-blue-800 dark:text-blue-300"
                  >
                    <span className="font-medium">
                      {t('questionnaire.budget.aiTip', 'AI Tip')}:
                    </span>{' '}
                    {tip}
                  </div>
                ))}
                {aiTips.warnings.map((w, i) => (
                  <div
                    key={i}
                    className="bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-lg p-3 text-sm text-amber-800 dark:text-amber-300"
                  >
                    <span className="font-medium">
                      {t('questionnaire.budget.aiWarning', 'Warning')}:
                    </span>{' '}
                    {w}
                  </div>
                ))}
                {aiTips.suggestions.map((s, i) => (
                  <div
                    key={i}
                    className="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3 text-sm text-gray-700 dark:text-gray-300"
                  >
                    <span className="font-medium">
                      {t('questionnaire.budget.aiSuggestion', 'Suggestion')}:
                    </span>{' '}
                    {s}
                  </div>
                ))}
              ...
              <div className="pt-6 border-t border-gray-200 dark:border-gray-700">
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6 text-center">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 max-w-md mx-auto">
                    {t(
                      'questionnaire.budget.generateDescription',
                      'Notre IA va analyser vos reponses et generer 3 concepts de cuisine a differents niveaux de budget (Economique, Confort, Premium).'
                    )}
                  </p>
