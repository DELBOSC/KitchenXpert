...
const KitchenCreate: React.FC = () => {
  ...
  const styleOptions = [
    { id: 'modern', name: t('kitchens.styles.modern', 'Modern'), description: 'Clean lines, minimal ornamentation' },
    { id: 'traditional', name: t('kitchens.styles.traditional', 'Traditional'), description: 'Classic details, warm tones' },
    { id: 'contemporary', name: t('kitchens.styles.contemporary', 'Contemporary'), description: 'Current trends, bold accents' },
    { id: 'transitional', name: t('kitchens.styles.transitional', 'Transitional'), description: 'Blend of traditional and modern' },
    { id: 'farmhouse', name: t('kitchens.styles.farmhouse', 'Farmhouse'), description: 'Rustic, cozy, natural materials' },
    { id: 'industrial', name: t('kitchens.styles.industrial', 'Industrial'), description: 'Raw materials, urban aesthetic' },
    { id: 'scandinavian', name: t('kitchens.styles.scandinavian', 'Scandinavian'), description: 'Light, airy, functional' },
    { id: 'mediterranean', name: t('kitchens.styles.mediterranean', 'Mediterranean'), description: 'Warm colors, ornate details' },
  ];
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      ...
      newErrors.name = t('kitchens.validation.nameMinLength', 'Kitchen name must be at least 2 characters');
    } else if (formData.name.length > 100) {
      newErrors.name = t('kitchens.validation.nameMaxLength', 'Kitchen name must be less than 100 characters');
    }
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="mb-6">
          <ol className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
            ...
            <li>/</li>
            <li>
              <Link to={`/projects/${projectId}`} className="hover:text-blue-600 dark:hover:text-blue-400">
                {isLoadingProject ? '...' : (projectName || t('projects.project', 'Project'))}
              </Link>
            </li>
            <li>/</li>
            <li className="text-gray-900 dark:text-white font-medium">{t('kitchens.createKitchen', 'New Kitchen')}</li>
          </ol>
        </nav>

        {/* Form Card */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('kitchens.createNew', 'Create New Kitchen')}</h1>
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            {t('kitchens.createDescription', 'Define the kitchen space dimensions and design style. You can refine details in the designer.')}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              />
              {errors.name && (
                <p id="kitchen-name-error" className="mt-1 text-sm text-red-600" role="alert">{errors.name}</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="flex gap-4">
                <label className={`flex items-center px-4 py-2 border rounded-lg cursor-pointer ${
                  formData.unit === 'meters' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600'
                }`}>
                  <input
                    type="radio"
                    name="unit"
                ...
                <label className={`flex items-center px-4 py-2 border rounded-lg cursor-pointer ${
                  formData.unit === 'feet' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600'
                }`}>
                  <input
                    type="radio"
                    name="unit"
            ...
            <section>
              <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
                {t('kitchens.roomDimensions', 'Room Dimensions')} <span className="text-red-500">*</span>
              </h2>

              <div className="grid gap-6 md:grid-cols-3">
                <div>
                  <label htmlFor="roomWidth" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  ...
                  />
                  {errors.roomWidth && (
                    <p id="roomWidth-error" className="mt-1 text-sm text-red-600" role="alert">{errors.roomWidth}</p>
                  )}
                </div>

                <div>
                  <label htmlFor="roomDepth" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  ...
                  />
                  {errors.roomDepth && (
                    <p id="roomDepth-error" className="mt-1 text-sm text-red-600" role="alert">{errors.roomDepth}</p>
                  )}
                </div>

                <div>
                  <label htmlFor="roomHeight" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  ...
                  />
                  {errors.roomHeight && (
                    <p id="roomHeight-error" className="mt-1 text-sm text-red-600" role="alert">{errors.roomHeight}</p>
                  )}
                </div>
              </div>
              ...
              {formData.roomWidth > 0 && formData.roomDepth > 0 && (
                <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-sm text-blue-800 dark:text-blue-300">
                    {t('kitchens.totalFloorArea', 'Total Floor Area')}: <strong>{calculateArea()} {formData.unit === 'meters' ? 'sq m' : 'sq ft'}</strong>
                  </p>
                </div>
              )}
            </section>
            ...
            <section>
              ...
              {errors.style && (
                <p className="mb-3 text-sm text-red-600" role="alert">{errors.style}</p>
              )}
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {styleOptions.map((style) => (
                  ...
                  >
                    ...
                    <h3 className="font-medium text-gray-900 dark:text-white text-sm">{style.name}</h3>
        ...
        <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <h3 className="font-medium text-blue-900 dark:text-blue-300 mb-2">{t('kitchens.tips.title', 'Tips for Kitchen Setup')}</h3>
          <ul className="text-sm text-blue-700 dark:text-blue-400 space-y-1">
            <li>- {t('kitchens.tips.accurateMeasure', 'Measure your room accurately for the best design results')}</li>
            <li>- {t('kitchens.tips.ceilingHeight', 'Include the actual ceiling height for proper cabinet planning')}</li>
            <li>- {t('kitchens.tips.styleFlexible', 'Your style choice guides AI suggestions but can be adjusted later')}</li>
            <li>- {t('kitchens.tips.designerEdit', 'All dimensions and details can be refined in the 3D designer')}</li>
          </ul>
        </div>
      </div>
    </div>
