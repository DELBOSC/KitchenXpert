...
function ScoreBar({ label, value }: { label: string; value: number }): React.ReactElement {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] font-medium text-gray-700 dark:text-gray-300 w-7 text-right">
        {value}
      </span>
    </div>
  );
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-600 rounded-xl overflow-hidden bg-white dark:bg-gray-800 hover:shadow-lg transition-shadow">
      ...
      <div className="px-4 py-3 space-y-1.5">
        <ScoreBar
          label={t('designer.gen.ergonomics', 'Ergonomie')}
          value={proposal.score.ergonomics}
        />
        <ScoreBar label={t('designer.gen.storage', 'Rangement')} value={proposal.score.storage} />
        <ScoreBar
          label={t('designer.gen.aesthetics', 'Esthetique')}
          value={proposal.score.aesthetics}
        />
        <ScoreBar
          label={t('designer.gen.space', 'Espace')}
          value={proposal.score.spaceUtilization}
        />
      </div>
