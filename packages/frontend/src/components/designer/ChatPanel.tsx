...
function mapItemType(item: string): string {
  const map: Record<string, string> = {
    meuble: 'base', cabinet: 'base', placard: 'wall',
...
function mapView(view: string): string {
  const map: Record<string, string> = {
    dessus: 'top', top: 'top', face: 'front', front: 'front',
...
const LOCAL_VOICE_COMMANDS: Array<{
  ...
  {
    patterns: [
      /(?:supprime|enlève|remove|delete)\s+(?:le |la |l')?(.*)/i,
    ],
    action: (match) => ({
      tool: 'remove_object',
  ...
  {
    patterns: [
      /(?:vue|view)\s+(dessus|top|face|front|droite|right|gauche|left|perspective|3d)/i,
    ],
    action: (match) => ({
      tool: 'switch_view',
  ...
  {
    patterns: [
      /(?:quel|what|combien|how much).*(?:budget|coût|cost|prix|price)/i,
    ],
    action: () => ({
  ...
  {
    patterns: [
      /(?:triangle|ergonomie|ergonomic)/i,
    ],
    action: () => ({
...
function extractSceneContext(engine: KitchenEngine): SceneContext {
  ...
  scene.traverse((child: THREE.Object3D) => {
    if (!child.userData.id || child.userData.type === 'wall' || child.userData.type === 'floor' || child.userData.isKitchenStructure) {
      return;
    }
...
export default function ChatPanel({ engine, onClose, onToolAction }: ChatPanelProps): React.ReactElement {
  ...
  const toolLabels: Record<string, string> = useMemo(() => ({
    ...
    analyze_work_triangle: t('designer.chat.tool.analyzeTriangle', 'Analyser le triangle de travail'),
  ...
  const executeLocalVoiceCommand = useCallback((result: LocalVoiceCommandResult) => {
  ...
  const toggleVoiceInput = useCallback(() => {
    ...
    recognition.lang = currentLang.includes('-') ? currentLang : `${currentLang}-${currentLang.toUpperCase()}`;
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800" role="complementary" aria-label={t('designer.chat.title', 'Chat IA')}>
      {/* Header */}
                  ...
                  sessions.map((session) => (
                    <button
                      ...
                      className={`w-full px-3 py-2 text-left text-xs hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors ${
                        session.id === sessionId ? 'bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300' : 'text-gray-700 dark:text-gray-300'
                      }`}
                    >
                      <div className="font-medium truncate">{session.title || t('designer.chat.untitled', 'Sans titre')}</div>
      ...
      {/* Messages area */}
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3" role="log" aria-live="polite" aria-label={t('designer.chat.messages', 'Messages')}>
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-4 py-6">
            <div className="text-center">
              <svg className="w-10 h-10 text-gray-300 dark:text-gray-600 mx-auto mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
                    ...
                    <div className="space-y-2">
                      {msg.toolUse.map((tool, toolIdx) => (
                        <ToolActionCard
                          ...
                          onDismiss={() => { /* noop - card hides itself */ }}
                          t={t}
        ...
        <div className="flex items-end gap-2">
          ...
          {voiceSupported && (
            <button
              ...
              aria-label={isListening ? t('chat.listening', 'Listening...') : t('chat.voiceInput', 'Voice input')}
