...
describe('HttpJsonFetcher', () => {
  ...
  it('envoie UA + Accept + headers par appel (ex. Origin) et parse le JSON', async () => {
    ...
    const out = await f.fetchJson<{ a: number }>('https://x/y', { headers: { Origin: 'https://x' } });
  ...
  it('throw sur réponse non-ok (status dans le message)', async () => {
    global.fetch = jest
      .fn()
      .mockResolvedValue({ ok: false, status: 503, statusText: 'Service Unavailable', json: async () => ({}) }) as unknown as typeof fetch;
