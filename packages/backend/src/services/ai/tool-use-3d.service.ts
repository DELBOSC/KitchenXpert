...
export class ToolUse3DService {
  ...
  private buildSceneDescription(context: SceneContext): string {
    ...
    const items = context.items
      .map(
        (i) =>
          `- [${sanitizePromptField(i.id, 64)}] ${sanitizePromptField(i.type)}${i.name ? ` (${sanitizePromptField(i.name)})` : ''} @ (${num(i.position.x).toFixed(0)}, ${num(i.position.y).toFixed(0)}, ${num(i.position.z).toFixed(0)})${i.dimensions ? ` [${num(i.dimensions.width)}x${num(i.dimensions.height)}x${num(i.dimensions.depth)}mm]` : ''}`
      )
      .join('\n');
