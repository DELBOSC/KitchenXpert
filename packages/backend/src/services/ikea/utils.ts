...
export function parsePrice(priceData: unknown): number {
  ...
  if (typeof priceData === 'object' && priceData !== null) {
    ...
    if ('amount' in obj) {
      return parsePrice(obj.amount);
    }
    if ('value' in obj) {
      return parsePrice(obj.value);
    }
    if ('price' in obj) {
...
export function extractImageUrl(media: unknown): string | undefined {
  ...
  if (typeof media === 'object' && media !== null) {
    ...
    if ('mainImageUrl' in obj) {
      return String(obj.mainImageUrl);
    }
    if ('S5' in obj) {
      return String(obj.S5);
    } // IKEA uses size codes like S5
    if ('S4' in obj) {
      return String(obj.S4);
    }
    if ('S3' in obj) {
      return String(obj.S3);
    }
  }
...
export function parseDimensions(raw: unknown): ParsedDimensions {
  ...
  const label = (obj.productMeasureReferenceText ?? obj.measurementText ?? obj.measureText) as
    | string
    | undefined;
  if (label && (out.width === undefined || out.depth === undefined || out.height === undefined)) {
    ...
    while ((m = DIM_TEXT_RE.exec(label))) {
      ...
      if ((letter === 'L' || letter === 'W') && out.width === undefined) {
        out.width = cm;
      } else if ((letter === 'P' || letter === 'D') && out.depth === undefined) {
        out.depth = cm;
      } else if ((letter === 'H' || letter === 'h') && out.height === undefined) {
        out.height = cm;
      }
    }
  }
  ...
  if (out.width === undefined && Array.isArray(obj.childItems)) {
    ...
    for (const child of children) {
      ...
      if (dims.width) {
        totalW += dims.width * qty;
        any = true;
      }
      if (dims.depth && dims.depth > maxD) {
        maxD = dims.depth;
      }
      if (dims.height && dims.height > maxH) {
        maxH = dims.height;
      }
    }
    if (any) {
      out.width = Math.round(totalW * 10) / 10;
      if (out.depth === undefined && maxD > 0) {
        out.depth = maxD;
      }
      if (out.height === undefined && maxH > 0) {
        out.height = maxH;
      }
    }
  }
