  ...
  cabinet: [
    {
      ...
      priceDiff: -30,
      inStock: true,
      reason: 'Same dimensions, IKEA equivalent with similar finish',
    },
    {
      sku: 'ALT-CAB-002',
      brand: 'Leroy Merlin',
      name: 'Delinia ID Base 60cm',
      matchScore: 0.85,
      priceDiff: -15,
      inStock: true,
      reason: 'Same dimensions, comparable quality from Leroy Merlin range',
    },
    {
      sku: 'ALT-CAB-003',
      brand: 'Castorama',
      name: 'GoodHome Caraway Base 60cm',
      matchScore: 0.78,
      priceDiff: -25,
      inStock: true,
      reason: 'Similar dimensions and style, budget-friendly alternative',
    },
  ],
  countertop: [
    {
      sku: 'ALT-CT-001',
      brand: 'IKEA',
      name: 'EKBACKEN Laminate Countertop',
      matchScore: 0.8,
      priceDiff: -80,
      inStock: true,
      reason: 'Same width, laminate alternative at lower price point',
    },
    {
      sku: 'ALT-CT-002',
      brand: 'Leroy Merlin',
      ...
      priceDiff: 20,
      inStock: true,
      reason: 'Same dimensions, quartz composite with similar appearance',
    },
  ],
  appliance: [
    {
      sku: 'ALT-APP-001',
      brand: 'Bosch',
      name: 'Serie 4 Built-in Oven',
      matchScore: 0.9,
      priceDiff: 50,
      inStock: true,
      reason: 'Same installation dimensions, equivalent energy rating',
    },
    {
      sku: 'ALT-APP-002',
      brand: 'Whirlpool',
      name: 'W Collection Built-in Oven',
      matchScore: 0.82,
      priceDiff: -40,
      inStock: true,
      reason: 'Compatible dimensions, similar features at lower price',
    },
  ],
  handle: [
    {
      sku: 'ALT-HDL-001',
      brand: 'IKEA',
      name: 'BAGGANÄS Handle 143mm',
      matchScore: 0.88,
      priceDiff: -5,
      inStock: true,
      reason: 'Similar style and finish, standard hole spacing',
    },
    {
      sku: 'ALT-HDL-002',
      brand: 'Leroy Merlin',
      name: 'GoodHome Annatto Bar Handle',
      matchScore: 0.85,
      priceDiff: -2,
      inStock: true,
      reason: 'Same finish and mounting, comparable design',
    },
  ],
};
...
function mockQuantity(sku: string, status: string): number {
  ...
  switch (status) {
    case 'in_stock':
      return 10 + (absHash % 90); // 10-99
    case 'low_stock':
      return 1 + (absHash % 5); // 1-5
    case 'out_of_stock':
      return 0;
    case 'backordered':
      return 0;
...
function mockStoreAvailability(
  ...
  return stores.map((store, idx) => {
    ...
    const inStock =
      overallStatus === 'in_stock' || (overallStatus === 'low_stock' && storeHash % 3 > 0);
    return {
      ...store,
      inStock,
...
export class StockCheckerService {
   ...
   */
  async checkAvailability(items: StockCheckItem[]): Promise<StockResult[]> {
   ...
   */
  async findAlternatives(item: AlternativeSearchItem): Promise<AlternativeProduct[]> {
    ...
    const alternatives = typeAlternatives
      ...
      .map((alt) => ({
        ...
        matchScore: Number(
          Math.min(
            1,
            Math.max(0, alt.matchScore + ((item.dimensions.width % 7) - 3) * 0.01)
          ).toFixed(2)
        ),
      }))
