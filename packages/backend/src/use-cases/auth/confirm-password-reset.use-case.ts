...
export class ConfirmPasswordResetUseCase implements UseCase<
...
> {
  ...
  async execute({
    token,
    newPassword,
  }: ConfirmPasswordResetInput): Promise<Result<{ userId: string }>> {
    const hashed = crypto.createHash('sha256').update(token).digest('hex');
    const record = await this.prisma.passwordResetToken.findFirst({
      where: { token: hashed, usedAt: null, expiresAt: { gt: new Date() } },
    });
    if (!record) {
      return err(DomainErrors.validation('Invalid or expired reset token'));
    }
