...
export class CollaborationRoomManager {
  ...
  joinRoom(socket: AuthenticatedSocket): void {
    ...
    if (!room) {
      room = this.createRoom(kitchenId);
    }
  ...
  handleMessage(socket: AuthenticatedSocket, message: WSMessage): void {
    ...
    switch (message.type) {
      case 'doc-update':
        this.handleDocUpdate(room, socket, message);
        break;
      case 'cursor-update':
        this.handleCursorUpdate(room, socket, message);
        break;
      case 'request-state':
        this.handleRequestState(room, socket);
        break;
      default:
  ...
  private createRoom(kitchenId: string): CollaborationRoom {
    const doc = new Y.Doc();
  ...
  private handleRequestState(room: CollaborationRoom, socket: AuthenticatedSocket): void {
    const stateVector = Y.encodeStateAsUpdate(room.doc);
    this.sendToSocket(socket, {
      type: 'full-state',
      kitchenId: room.kitchenId,
