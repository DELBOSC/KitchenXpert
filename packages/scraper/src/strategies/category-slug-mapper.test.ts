...
function up(over: Partial<UnifiedProduct> = {}): UnifiedProduct {
  return {
    sku: 'X', name: 'Produit', brand: 'B', type: 'cabinet',
    widthMm: null, heightMm: null, depthMm: null, dimensionConfidence: 0,
    priceEurCents: null, currency: 'EUR', sourceLevel: 1,
    sourceUrl: 'https://x.test/p', lastVerifiedAt: new Date(),
    specifications: {},
    ...over,
  };
}
...
describe('resolveCategorySlug', () => {
  it('EPREL applianceGroup -> catégorie (explicit)', () => {
    ...
    for (const [group, slug] of cases) {
      const r = resolveCategorySlug(up({ type: 'appliance', specifications: { applianceGroup: group } }));
  ...
  it('applianceGroup prioritaire sur productType', () => {
    ...
    const r = resolveCategorySlug(up({ type: 'cabinet', specifications: { applianceGroup: 'ovens' } }));
    expect(r.slug).toBe('electromenager-cuisson');
    expect(r.detection).toBe('explicit');
  });

  it('worktop -> plans-de-travail (explicit)', () => {
    expect(resolveCategorySlug(up({ type: 'worktop' }))).toEqual({ slug: 'plans-de-travail', detection: 'explicit' });
  });
  ...
  it('facade -> facades (explicit)', () => {
    expect(resolveCategorySlug(up({ type: 'facade' }))).toEqual({ slug: 'facades', detection: 'explicit' });
  });
  ...
  it('appliance SANS group -> electromenager-cuisson (inferred)', () => {
    expect(resolveCategorySlug(up({ type: 'appliance', specifications: {} }))).toEqual({
      slug: 'electromenager-cuisson', detection: 'inferred',
    });
  });

  it('cabinet : détection colonne / haut / mural / bas / fallback (inferred)', () => {
    expect(resolveCategorySlug(up({ type: 'cabinet', name: 'Structure pour colonne L. 200' })).slug).toBe('colonnes');
    expect(resolveCategorySlug(up({ type: 'cabinet', name: 'Meuble haut 60' })).slug).toBe('meubles-hauts');
    expect(resolveCategorySlug(up({ type: 'cabinet', name: 'Caisson mural blanc' })).slug).toBe('meubles-hauts');
    expect(resolveCategorySlug(up({ type: 'cabinet', name: 'Caisson bas 80' })).slug).toBe('meubles-bas');
    expect(resolveCategorySlug(up({ type: 'cabinet', name: 'Caisson de cuisine GoodHome' })).slug).toBe('meubles-bas'); // fallback
    expect(resolveCategorySlug(up({ type: 'cabinet', name: 'X' })).detection).toBe('inferred');
  });

  it('colonne prioritaire sur bas/haut (ordre)', () => {
    expect(resolveCategorySlug(up({ type: 'cabinet', name: 'Colonne meuble bas' })).slug).toBe('colonnes');
  });

  it('type non couvert -> null (storage/handle/lighting/unknown)', () => {
    for (const t of ['storage', 'handle', 'lighting', 'accessory', 'decoration', 'unknown'] as const) {
