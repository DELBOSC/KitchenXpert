 ...
 *
 * Replaces the dead HTML/JSON-LD path of the legacy scrapers/ikea.ts (kept
 * untouched until roadmap step d) with IKEA's internal search API, proven live
 * 14/06 (cf §15.7): sik.search.blue.cdtapps.com/fr/fr/search-result-page.
 * No Cloudflare on that host, structured JSON. Cascade level N2 (§15.8 P1).
 *
 * Business logic (type detection, dimension parsing) ported from the legacy
 * ikea.ts but reshaped to emit the unified schema with dimensionConfidence.
 */
