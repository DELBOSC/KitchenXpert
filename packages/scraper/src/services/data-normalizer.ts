...
export class DataNormalizer {
  ...
  normalizeCabinet(input: Partial<CreateCabinetInput>): NormalizationResult<CreateCabinetInput> {
    ...
    try {
      ...
      const dimensions = this.normalizeDimensions(input.width, input.height, input.depth, 'cabinet');
      if (dimensions.width !== input.width) changes.push(`Adjusted width: ${input.width} -> ${dimensions.width}`);
      if (dimensions.height !== input.height) changes.push(`Adjusted height: ${input.height} -> ${dimensions.height}`);
      if (dimensions.depth !== input.depth) changes.push(`Adjusted depth: ${input.depth} -> ${dimensions.depth}`);
  ...
  normalizeWorktop(input: Partial<CreateWorktopInput>): NormalizationResult<CreateWorktopInput> {
    ...
    try {
      ...
      const thicknesses = input.thicknesses?.map(t => this.normalizeWorktopThickness(t) || t) || [];
  ...
  normalizeAppliance(input: Partial<CreateApplianceInput>): NormalizationResult<CreateApplianceInput> {
    const errors: string[] = [];
    const warnings: string[] = [];
    const changes: string[] = [];
  ...
  normalizeAccessory(input: Partial<CreateAccessoryInput>): NormalizationResult<CreateAccessoryInput> {
    const errors: string[] = [];
    const warnings: string[] = [];
    const changes: string[] = [];
  ...
  private normalizeColors(colors: Array<{ name: string; code?: string; hex?: string }>): Array<{ name: string; code?: string; hex?: string }> {
    return colors.map((c) => ({
      ...c,
