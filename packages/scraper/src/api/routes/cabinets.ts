  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      const params: CabinetSearchParams = {
        ...
        categories: req.query.category ? String(req.query.category).split(',') as CabinetCategory[] : undefined,
  ...
  router.get('/by-reference/:brandId/:reference', async (req: Request, res: Response) => {
    try {
      ...
      if (!brandId || !reference) {
        res.status(400).json({ error: 'Bad Request', message: 'Brand ID and reference are required' });
        return;
      }
