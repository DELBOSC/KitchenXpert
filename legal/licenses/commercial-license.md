# KitchenXpert Commercial License Agreement

**Effective Date:** Upon subscription activation

**Version:** 1.0

**Last Updated:** January 2026

---

## INTRODUCTION

This Commercial License Agreement ("License" or "Agreement") governs the use of the KitchenXpert platform and services ("Platform" or "Service") by businesses, professionals, and commercial entities ("Licensee" or "You"). By subscribing to any paid tier of KitchenXpert, you agree to be bound by the terms of this License.

**Licensor:**
KitchenXpert SAS
42 Rue de la Cuisine
75008 Paris, France
Email: legal@kitchenxpert.com

---

## 1. GRANT OF LICENSE

### 1.1 License Grant

Subject to the terms and conditions of this Agreement and payment of applicable fees, KitchenXpert grants Licensee a non-exclusive, non-transferable, limited license to access and use the Platform for commercial purposes during the subscription term.

### 1.2 Scope of License

This License permits Licensee to:

- Access and use the Platform through authorized user accounts
- Design kitchen layouts and spaces for commercial purposes
- Utilize AI-powered design recommendations and suggestions
- Generate 3D visualizations and renderings for client projects
- Export designs in available formats for business use
- Access the product catalog and integrate products into designs

### 1.3 Non-Transferability

This License is granted solely to the Licensee identified in the subscription account. Licensee may not transfer, sublicense, or assign this License to any third party without KitchenXpert's prior written consent.

### 1.4 Number of Authorized Users

The number of authorized users permitted under this License depends on the subscription tier, as detailed in Section 2 below.

---

## 2. LICENSE TIERS

KitchenXpert offers three commercial license tiers, each designed to meet different business needs:

### 2.1 Pro License (Single User)

**Price:** EUR 29 per month (billed monthly) or EUR 290 per year (billed annually)

**Authorized Users:** One (1) named user

**Features:**

- Full access to all design tools and AI features
- Commercial use rights for client projects
- High-resolution exports without watermarks
- Access to complete product catalog
- Priority rendering queue
- Email support with forty-eight (48) hour response time
- Five hundred (500) GB cloud storage for designs
- Unlimited design projects
- Client presentation mode
- Export formats: PDF, PNG, JPG, DXF

**Best For:** Individual designers, freelancers, and small studios

### 2.2 Business License (Team)

**Price:** EUR 199 per month (billed monthly) or EUR 1,990 per year (billed annually)

**Authorized Users:** Up to ten (10) named users

**Features:**

All Pro features, plus:

- Team collaboration tools
- Shared design libraries and templates
- Team workspace with project management
- Role-based access control
- Centralized billing and account management
- Priority email and chat support with twenty-four (24) hour response
- Two (2) TB shared cloud storage
- Custom branding on exports
- Brand asset library
- Client portal for design sharing and approval
- Advanced analytics and reporting
- Export formats: All Pro formats plus 3DS, OBJ, FBX

**Best For:** Design firms, kitchen retailers, and interior design agencies

### 2.3 Enterprise License (Unlimited)

**Price:** Custom pricing (contact sales@kitchenxpert.com)

**Authorized Users:** Unlimited (organization-wide)

**Features:**

All Business features, plus:

- Unlimited users across the organization
- Full API access for custom integrations
- Single Sign-On (SSO) integration
- Dedicated account manager
- Four (4) hour response time for critical issues
- Quarterly business reviews
- Custom training and onboarding
- Priority access to new features
- Unlimited cloud storage
- Custom data retention policies
- Advanced security controls
- SLA with 99.95% uptime guarantee
- Custom contract terms available
- White-label options (separate agreement)

**Best For:** Large enterprises, franchise operations, and major retailers

---

## 3. PERMITTED USES

### 3.1 Client Work

Licensee is expressly permitted to:

- Design kitchens for paying clients as part of professional services
- Create design proposals and presentations for client pitches
- Use AI recommendations to enhance client project outcomes
- Incorporate partner catalog products into client designs
- Provide design consultation services using Platform capabilities

### 3.2 Business Documentation

Licensee may use Platform outputs for:

- Client proposals and quotations
- Marketing materials showcasing design capabilities
- Portfolio presentations and case studies
- Trade show displays and demonstrations
- Social media content featuring designs (with attribution encouraged)

### 3.3 Client Presentations

Licensee may:

- Present designs to clients in digital or printed format
- Share designs via the client portal (Business and Enterprise tiers)
- Export high-resolution images for client review
- Create virtual walkthroughs and 3D presentations
- Print designs for client meetings and approvals

### 3.4 AI Feature Usage

Licensee may fully utilize AI-powered features including:

- Automated design suggestions and optimization
- Style matching and recommendation engines
- Space planning algorithms
- Product recommendation systems
- Cost estimation tools
- Accessibility compliance checking

---

## 4. RESTRICTIONS

### 4.1 Prohibited Activities

Licensee shall NOT:

- **Resell or Redistribute Access:** Sell, lease, or otherwise provide access to the Platform to third parties
- **White-Labeling:** Present the Platform as Licensee's own proprietary software (except under separate Enterprise white-label agreement)
- **Competitive Development:** Use the Platform, its features, or outputs to develop or enhance a competing product or service
- **Automated Bulk Operations:** Use automated tools, bots, or scripts to generate designs in bulk without explicit authorization
- **Circumvent Limitations:** Attempt to bypass user limits, feature restrictions, or security measures
- **Reverse Engineering:** Decompile, disassemble, or reverse engineer any Platform component
- **Data Extraction:** Systematically extract or scrape data from the Platform beyond normal usage

### 4.2 Attribution Requirements

For publicly shared designs (social media, websites, publications):

- **Pro Tier:** Attribution encouraged but not required: "Designed with KitchenXpert"
- **Business Tier:** No attribution required
- **Enterprise Tier:** No attribution required; custom branding options available

### 4.3 User Account Restrictions

- Each authorized user must have a unique, individual account
- Account credentials may not be shared between individuals
- Licensee is responsible for all activity under its user accounts
- Licensee must promptly notify KitchenXpert of any unauthorized access

---

## 5. CLIENT DELIVERABLES

### 5.1 Ownership of Client Work

Licensee retains full ownership of all original design work created using the Platform for clients. This includes:

- Kitchen layout designs and configurations
- Custom design decisions and selections
- Client-specific modifications and adaptations
- Design documentation and specifications

### 5.2 Client Rights

Licensee may grant clients the following rights to designs:

- Right to use designs for implementation purposes
- Right to share designs with contractors and installers
- Right to retain designs for personal records
- No right to commercial redistribution of designs

### 5.3 Export Watermarks

- **Free Tier:** Exports include KitchenXpert watermark
- **Pro, Business, and Enterprise Tiers:** Exports are watermark-free
- Custom branding available on Business and Enterprise tiers

### 5.4 Design File Delivery

Licensee may provide clients with:

- Exported design files in available formats
- 3D model files (where permitted by tier)
- Material and product specifications
- Cost estimates and bills of materials

---

## 6. SUPPORT

### 6.1 Support Channels and Response Times

| Tier | Channels | Response Time |
|------|----------|---------------|
| Pro | Email | 48 hours |
| Business | Email + Live Chat | 24 hours |
| Enterprise | Email + Chat + Phone | 4 hours (critical) |

### 6.2 Support Scope

Support includes assistance with:

- Platform functionality and features
- Technical issues and troubleshooting
- Account and billing inquiries
- Best practices and usage guidance

Support does NOT include:

- Design advice or aesthetic recommendations
- Third-party software integration troubleshooting
- On-site installation or hardware support
- Custom development (available separately for Enterprise)

### 6.3 Business Hours

- Standard support: Monday-Friday, 09:00-18:00 CET
- Enterprise critical issues: 24/7

---

## 7. TERM AND SUBSCRIPTION

### 7.1 Subscription Term

- **Monthly subscriptions:** Auto-renew monthly
- **Annual subscriptions:** Auto-renew annually
- **Enterprise contracts:** As specified in Order Form

### 7.2 Auto-Renewal

Subscriptions automatically renew at the end of each billing period unless:

- Licensee cancels before the renewal date
- Licensee downgrades to a different tier
- Either party terminates for cause

### 7.3 Cancellation Policy

**Monthly Subscriptions:**
- Cancel anytime via account settings
- Access continues until end of current billing period
- No refunds for partial months

**Annual Subscriptions:**
- Cancel anytime via account settings
- Access continues until end of annual period
- No pro-rata refunds (see Refund Policy for exceptions)

**Enterprise Contracts:**
- Governed by individual contract terms
- Typically require written notice 90 days before renewal

### 7.4 Subscription Changes

- **Upgrades:** Effective immediately; pro-rated billing
- **Downgrades:** Effective at next billing cycle
- **User additions (Business/Enterprise):** Immediate; pro-rated billing

---

## 8. AUDIT RIGHTS

### 8.1 Compliance Verification

KitchenXpert reserves the right to verify Licensee's compliance with this License, including:

- Number of users accessing the Platform
- Compliance with usage restrictions
- Adherence to permitted use guidelines

### 8.2 Audit Process

- KitchenXpert shall provide at least thirty (30) days written notice before any audit
- Audits shall be conducted during normal business hours
- Audits shall not unreasonably interfere with Licensee's business operations
- KitchenXpert shall bear audit costs unless material non-compliance is discovered

### 8.3 Remediation

If an audit reveals License violations:

- Licensee must promptly remedy the violation
- Licensee may be required to pay for unauthorized usage
- Repeated violations may result in License termination

---

## 9. INTELLECTUAL PROPERTY

### 9.1 Platform Ownership

KitchenXpert retains all right, title, and interest in:

- Platform software, code, and architecture
- AI models, algorithms, and methodologies
- User interface designs and visual elements
- Documentation and training materials
- Trademarks, logos, and brand assets

### 9.2 Licensee Content

Licensee retains ownership of:

- Original design work created on the Platform
- Client information and project data
- Custom templates and design libraries
- Business data and analytics derived from usage

### 9.3 Product Catalog Content

Product information and images from the catalog remain the property of respective manufacturers and partners. Licensee's right to use catalog content is limited to designing projects on the Platform.

---

## 10. DATA AND PRIVACY

### 10.1 Data Ownership

- Licensee owns all design data and client information entered into the Platform
- KitchenXpert may use anonymized, aggregated data for service improvement

### 10.2 Data Security

KitchenXpert implements industry-standard security measures including:

- Encryption of data in transit and at rest
- Regular security audits and penetration testing
- Access controls and authentication mechanisms
- Backup and disaster recovery procedures

### 10.3 Data Export

Licensee may export their data at any time through:

- Built-in export functionality
- Data export request to support (comprehensive export)
- API access (Enterprise tier)

### 10.4 Data Retention

- Active accounts: Data retained indefinitely
- Cancelled accounts: Data retained for 30 days, then deleted
- Enterprise: Custom retention policies available

---

## 11. WARRANTIES AND DISCLAIMERS

### 11.1 Limited Warranty

KitchenXpert warrants that:

- The Platform will perform substantially in accordance with documentation
- Security measures meet industry standards
- Support will be provided as described

### 11.2 Disclaimer

EXCEPT AS EXPRESSLY PROVIDED HEREIN, THE PLATFORM IS PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND. KITCHENXPERT DISCLAIMS ALL IMPLIED WARRANTIES, INCLUDING WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NON-INFRINGEMENT.

### 11.3 Third-Party Products

KitchenXpert does not warrant:

- Third-party products displayed in the catalog
- Accuracy of manufacturer specifications
- Availability or pricing of catalog products

---

## 12. LIMITATION OF LIABILITY

### 12.1 Liability Cap

KITCHENXPERT'S TOTAL LIABILITY SHALL NOT EXCEED THE FEES PAID BY LICENSEE IN THE TWELVE (12) MONTHS PRECEDING THE CLAIM.

### 12.2 Exclusion of Damages

IN NO EVENT SHALL KITCHENXPERT BE LIABLE FOR INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, INCLUDING LOSS OF PROFITS, DATA, OR BUSINESS OPPORTUNITIES.

### 12.3 Essential Purpose

THESE LIMITATIONS APPLY EVEN IF ANY REMEDY FAILS OF ITS ESSENTIAL PURPOSE.

---

## 13. TERMINATION

### 13.1 Termination by Licensee

Licensee may terminate by cancelling the subscription through account settings or by contacting billing@kitchenxpert.com.

### 13.2 Termination by KitchenXpert

KitchenXpert may terminate this License if Licensee:

- Fails to pay fees when due
- Violates License restrictions
- Breaches any material term
- Uses the Platform for illegal purposes

### 13.3 Effect of Termination

Upon termination:

- Access to the Platform immediately ceases
- Licensee may export data for 30 days post-termination
- Outstanding fees become immediately due
- Provisions that should survive termination shall survive

---

## 14. GENERAL PROVISIONS

### 14.1 Governing Law

This License is governed by the laws of France.

### 14.2 Dispute Resolution

Disputes shall be resolved through negotiation, then mediation in Paris, then arbitration under ICC rules.

### 14.3 Entire Agreement

This License, together with the Terms of Service and Privacy Policy, constitutes the entire agreement.

### 14.4 Amendments

KitchenXpert may update this License with thirty (30) days notice. Continued use constitutes acceptance.

### 14.5 Severability

If any provision is unenforceable, the remaining provisions continue in effect.

### 14.6 No Waiver

Failure to enforce any provision does not waive that right.

---

## 15. CONTACT INFORMATION

For questions regarding this Commercial License:

**KitchenXpert SAS**
42 Rue de la Cuisine
75008 Paris, France

Email: legal@kitchenxpert.com
Sales: sales@kitchenxpert.com
Support: support@kitchenxpert.com

---

*Document Version: 1.0 | Last Updated: January 2026*

*This Commercial License Agreement is subject to KitchenXpert's Terms of Service and Privacy Policy, which are incorporated herein by reference.*
