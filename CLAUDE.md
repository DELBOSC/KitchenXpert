...
- [ ] **(question PRODUIT ouverte, pas une dette code) — palier 5/h anonyme largement caduc sur `/ai-chat`** : l'audit #217 a montré que **toutes** les routes `/ai-chat` (et `/ai-search`) sont `authenticate`/401 → **aucun coût Anthropic n'est atteignable anonymement** (un anonyme est 401 avant tout handler). Le palier « 5/h anonyme » de `aiUnauthRateLimiter` ne protège donc que du **flood de 401** (qu'un limiter général couvre déjà). **Si décision produit « jamais d'essai/usage anonyme »** → retirer `aiUnauthRateLimiter` du mount (Option C, ~1-2 lignes : `router.use('/ai-chat', aiChatRoutes)` + s'appuyer sur `aiRateLimiter` 20/h per-user + limiter général). **NON tranché** : c'est un choix produit, pas une correction technique — à arbitrer (garder un palier anonyme symbolique vs simplifier).
