...
- [ ] **Chantier formatage repo-wide** (la seule cause de rouge restante du job Lint) : `prettier --write` sur ~1299 fichiers (mesuré) + renommer `packages/frontend/.prettierrc.js` -> `.cjs` (ESM) + fixer parse-error `config/linters/stylelintrc.js` + créer `.prettierignore` (exclure dist/.astro/coverage — mesurer le périmètre réel hors artefacts AVANT le `--write`). Verdirait le job Lint entièrement. Mécanique mais volumineux : à faire à froid.
...
## 13. État d'avancement (snapshot 26/06/2026)
...
**Pendings P7.** (1) **Validation conversationnelle** via le frontend (route `POST /api/v1/ai-chat/shopping` authentifiée — tester en UI, pas en curl) : vérifier dans les logs que le LLM appelle `resolve_colors` et ne propose que les couleurs retournées. (2) UI color-picker (greenfield, mount `ChatPanel`). (3) `bom-generator` : **fait pour les lignes catalog** (BOM-a, §15.8.8 — vrais SKU/prix DB) ; reste BOM-b (matcher pour les lignes config).
...
**Pendings BOM.** (1) **BOM-b** : mapper les lignes `estimated` config → vrais produits via le matcher P6 (`DesignCatalogMatcher`) — ce sera son **1er call-site prod** (comme #199 pour le résolveur). (2) Cleanup : `SYSTEM_PROMPTS.BOM_GENERATOR` est désormais une constante orpheline (plus aucun appelant) → à retirer dans un cleanup isolé.
