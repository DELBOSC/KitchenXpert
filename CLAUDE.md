...
- `main` (HEAD `86acee0`, post-couche-6 03/06 — PRs #80/#81 mergées ; #75/#76/#77/#78 antérieures du 02/06)
- `ci/e2e-build-backend` (commit `b919c1b`, local + remote) — **PR #79 OUVERTE/HELD** (step Build backend e2e). À merger quand couche 7b sera réglée (cf §11 P3).
...
- `docs/session-02-06-couche6-7b` (branche courante, ce doc)

Branches mergées et supprimées 02-03/06 (refs prunés au `--delete-branch`) : `chore/align-aws-sdk-presigner` (#80), `chore/add-axe-core-playwright` (#81), `ci/turbo-frontend-ci` (#75), `ci/backend-ci-scripts-and-db-env` (#76), `chore/remove-orphan-submodule` (#77), `docs/session-02-06-cluster-cleanup-ci` (#78). Antérieures 31/05 prunées également.

Prochaine cible : (a) **couche 7b — chantier E2E dédié** (29/29 flux KO, signature `waitForLoadState`/networkidle → investiguer polling LiveCounter `/api/v1/stats/public` ou WebSocket collab ; ~4-8h) ; (b) **Lot 2 Dependabot** (Actions + dotenv) débloqué (facturation OK), PR #15 dotenv prouvée safe ; (c) **dettes CI 02/06** (vitest coverage-v8, ESLint front+back, security audit, 🚨 false green pnpm filter, build:backend turbo) ; (d) **§14.1 sécurité secrets** — bloquant prod. Couche 6 résolue (#79 held jusqu'à 7b) ; CodeQL (#2) reporté au passage repo public.
...
*Dernière mise à jour : 03/06/2026 (matin) — **Couche 6 backend startup E2E RÉSOLUE et PROUVÉE** hier soir 02-03/06 via #80 (align AWS SDK presigner, fix TS2345 cascade) + #81 (@axe-core/playwright devDep manquante) mergés. PR #79 (step Build backend e2e) ouverte/held par décision (rationale économique : sinon e2e = 25 min rouge sur chaque PR jusqu'à résolution couche 7b). **Nouvelle dette MAJEURE §11 P3 : Couche 7b** — suite E2E 29/29 flux KO systémique (signature `waitForLoadState`/networkidle, piste polling LiveCounter ou WebSocket), chantier dédié ~4-8h. Pattern audit-first a invalidé 100% des hypothèses initiales H1/H2/H3 → diagnostic par preuve uniquement. main HEAD = `86acee0`. Branche courante : `docs/session-02-06-couche6-7b`.*
