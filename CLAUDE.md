...
- [⚠️ QUASI-TERMINÉ — 10/06, 8/10 traités, 2 majeures restantes] **Audit `pnpm.overrides`** : bloc racine audité entièrement le 10/06 (10 entrées), **5 PRs mergées** (#118/#120/#121/#122 + doc #119). **Résultat combiné : vulns repo 101 → 50** (critical 6→2, high 46→21, moderate 41→21). **Bilan par entrée** : (a) ✅ **jspdf `^3.0.0` toxique — FIXÉ #118** : forçait jspdf 3.0.4 (CRITICAL path-traversal) alors que code+dep veulent v4 ; override périmé → retiré, résout 4.2.1. **4ᵉ override toxique du commit `edb726b`** (après path-to-regexp #47, minimatch #116). (b) ✅ **axios `^1.12.0`→`^1.17.0` — FIXÉ #120** (pin inefficace, 1.13.2 encore `<1.15.2` vuln). (c) ✅ **nodemailer `^7.0.4`→`^8.0.10` + dep backend `^6.9.8`→`^8.0.0` — FIXÉ #121** (`<8.0.4` SMTP injection ; v6→v8 mais API core stable, build+34 tests verts). (d) ✅ **3 pins périmés rafraîchis — FIXÉ #122** (same-major, 2 criticals) : protobufjs `^7.4.0`→`^7.6.3` (était 7.5.4, critical `<7.5.5` à un patch près), basic-ftp `^5.0.5`→`^5.3.1` (critical `<5.2.0`, patch v5), follow-redirects `^1.15.9`→`^1.16.0`. (e) ✅ **dompurify (3.4.7) + undici (6.26.0)** : réellement patchés, laissés tels quels. (f) ⏳ **2 RESTANTS = majeures sur transitifs** : `tar` v6→v7 (high `<7.5.8`), `fast-xml-parser` v4→v5 (moderate `<5.7.0`) — besoin analyse compat consommateur transitif avant bump. **Découverte clé** : la catégorie « 7 pins légitimes » était fausse — `pnpm audit` a montré **4/7 eux-mêmes vulnérables** (pins qui décaient). Règle confirmée : un override doit avoir (a) un consommateur réel, (b) une raison CVE, (c) **pointer une version RÉELLEMENT patchée** — vérifier par `pnpm audit`, pas juste « consommateur + raison ».
...
- [ ] **CI ESLint — crash RÉSOLU (#116), ~1551 erreurs réelles RESTANTES** : le « rouge ESLint » était d'abord un **CRASH** (`minimatch is not a function`, exit 2) — l'override toxique `minimatch:^9.0.5` (commit `edb726b`, même origine que l'override path-to-regexp retiré #47) forçait minimatch v9 (export objet) sur eslint@8.57/test-exclude@6 qui attendent l'API v3 callable. **Crash résolu 10/06 (#116, `57958c3`)** : override retiré → minimatch 3.1.5 (callable+ReDoS-safe). eslint tourne désormais. **MAIS révèle ~1551 erreurs lint réelles, 0% auto-fixable** (no-unsafe-member-access ×371, no-unsafe-assignment ×307, prefer-nullish-coalescing ×178, no-unescaped-entities ×176, no-floating-promises ×78…) = **chantier manuel massif** → décision stratégie (fix par lots / relaxer des règles en warning / ajuster eslintrc). Session dédiée.
