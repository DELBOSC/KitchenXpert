...
**Override d'apparence via `className` (depuis #235)** : `cn()` ([ui/_utils.ts](packages/frontend/src/components/ui/_utils.ts)) résout les conflits Tailwind via `tailwind-merge` → un `className` de call-site **écrase** la classe bakée de même famille (radius/border/bg/shadow/…) au lieu de s'y ajouter. C'est ce qui rend « étendre via props » réellement applicable **même quand on veut un rendu ≠ du variant par défaut** (ex : `<Card className="rounded-xl bg-transparent bg-gradient-to-r …">` écrase le `rounded-2xl`+bg baké de Card). Il n'y a **plus** de verrou d'infra : la règle §4 est nécessaire **et** suffisante.

**Règle de non-dérive (migration vers primitive)** : « zéro dérive visuelle » vise le **rendu perçu** — géométrie (radius, taille, position), couleur, espacement. Un **diff sous-pixel d'anti-aliasing** dû au changement de modèle de boîte d'une primitive (`inline-flex` de `<Button>` vs `<button>` natif) est **attendu et acceptable** lors d'une migration Button/Card — sinon aucune primitive ne serait jamais adoptable. Ce qui reste un **échec** : tout diff de **géométrie, couleur ou espacement**. Référence prouvée : migration SandboxMigrationBanner (wrapper Card = **0 px** ; boutons = 787 px / 1% de sous-pixel AA accepté — cf §11 P2).
...
- [x] **Infra `cn()` → tailwind-merge — FAIT (#235)** : `ui/_utils.ts` `cn()` passe de `join(' ')` additif à `twMerge` (même signature). Un `className` de call-site **écrase** désormais la classe bakée de même famille → §4 « étendre par props » fonctionne pleinement (plus de verrou d'infra additif — cf §4). Blast radius audité #233 = **1 seul** changement voulu (Select de tri CatalogPage : `w-auto` gagne enfin sur `w-full` baké → dropdown compact ; `!important` sur `h-9` devenu superflu → retiré). Filet `primitives-visual` (#234, 13 primitives sensibles) prouve automatiquement la non-régression ; **+6,77 Ko gzip** (tailwind-merge **2.6.1**, v2.x — v3=Tailwind v4 incompatible avec `^3.4.1`). Débloque la migration ci-dessous.
- [x] `SandboxMigrationBanner.tsx` : **migré vers primitives `Card` + `Button`** (débloqué par #235). Wrapper `<div>`→`<Card>` (rendu **pixel-parfait, 0 diff** : les overrides `className` `rounded-xl`/`border-indigo-400/20`/`bg-transparent`+gradient écrasent le look baké de Card via twMerge) ; 3 `<button>`→`<Button>` (primary/outline/ghost + `className` ramenant le look pill `rounded-full`/halo/`h-8` vers le `rounded-lg` plat d'origine). Handlers `handleImport`/`handleDelete` **intacts** → events A/B `sandbox_signup_completed`(+`_ab`) préservés (**test existant 3/3 vert, NON modifié**). Diff visuel **accepté** : 787 px / 1% de **sous-pixel AA** sur les glyphes des boutons (`inline-flex` de Button vs `<button>` natif), invisible à l'œil, irréductible sans renoncer à Button (cf règle de non-dérive §4). `primitives-visual` vert (Button/Card utilisés, non modifiés). **Dernière dette P2 « composant » close.**
...
- [x] **Test `payment-routes` FLAKY — RÉSOLU (13/07, #240)**. `Payment Routes > Edge Cases > should handle concurrent payment intent creation` : rouge ~1 fois sur 3 en suite complete, vert en isole (64/64). **Cause (audit-first) : PAS un etat partage entre fichiers** (Jest isole les registres par fichier, et `jest.clearAllMocks()` tourne en `beforeEach`) — c'est une **course interne au test**. Il chainait `mockResolvedValueOnce(pi_1).mockResolvedValueOnce(pi_2)`, qui sert les valeurs dans l'ordre ou le mock est **APPELE**. Avec deux requetes concurrentes (`Promise.all`), cet ordre n'est pas garanti : sous charge (suite complete, workers paralleles) la requete #2 double la #1 et recupere `pi_1`. **Fix** : le mock repond a la **requete** (`mockImplementation` corrélé sur `amount`), plus a l'ordre d'appel. Le test devient **deterministe ET plus fort** — il asserte enfin ce qu'il pretend (deux requetes concurrentes recoivent CHACUNE son intent, sans cross-talk), ce que la version ordre-dependante ne pouvait pas distinguer d'un jitter. **Preuve** : baseline 1 echec / 3 runs ; apres fix **5 runs consecutifs verts** ; **controle negatif** (correlation inversee) → le test echoue bien, donc il n'est pas devenu complaisant. **Acquit (cf reflexe §10.1)** : un gate qui ment une fois sur deux est presque pire qu'un rouge permanent — on prend l'habitude de « relancer pour voir », et le jour ou c'est une vraie regression, on relance, ca passe, on merge. Sur des routes de **paiement**, c'etait l'endroit le plus cher possible pour prendre cette habitude. Et contre-intuitivement, le fichier qui echouait **etait** le fautif : la cause etait locale, seul le **declencheur** etait global (la charge).
...
- [ ] **⬆️ RE-PRIORISÉE (12/07) — Recherche catalogue full-text : l'assistant IA est BRIDÉ par son absence.** La recherche produit est un `contains` **littéral** (substring), pas du full-text : `prisma.product.findMany({ name: { contains: q } })`. Tant que le catalogue n'était consulté que **par filtres UI**, le `contains` suffisait. **Depuis le câblage du tool `searchCatalog` (Palier 0), un LLM y envoie du LANGAGE NATUREL** → une requête un peu longue tombe sur le mur. **Prouvé en live** : `"façade blanche cuisine"` → **0 résultat** (le LLM a dû se corriger seul en `"façade blanche"` → 5 produits réels). Un utilisateur qui formule naturellement heurtera ça — et l'échec est **silencieux** (l'assistant dit honnêtement « rien trouvé », donc aucune alerte : juste un assistant inutile). **La recherche full-text devient une brique PRODUIT, plus une dette d'infra.** Action : vérifier que l'index GIN français (`gin_trgm_ops` ou `to_tsvector('french', ...)`) est présent côté Supabase après `prisma db push` (`db push` ne porte pas les index hors-schéma → probablement perdu à la migration), via `\d table_name` sur la base, puis poser une migration Prisma explicite ou un seed SQL. Puis basculer `buildProductWhere` (`catalog-search.service.ts`) du `contains` vers la recherche full-text.
- [ ] **Filtre catégorie du tool `searchCatalog` : 2 valeurs sans mapping.** L'enum du tool (`cabinet|appliance|worktop|splashback|sink|tap|accessory`) est ponté vers les slugs FR réels par `buildTypeWhereOr` (`catalog-type-mapping.ts`) — mais **`splashback` et `accessory` n'y sont pas** → fallback legacy `category.name contains` → très probablement 0 résultat. **Honnête** (aucune invention, l'assistant dit « rien trouvé ») mais **sous-sert**. Soit ajouter les slugs correspondants au mapping (s'ils existent en DB), soit retirer ces 2 valeurs de l'enum du tool.
...
- [x] **`tools` passé en `unknown[]` → le compilateur ne garde PAS le paramètre le plus critique (trouvé 14/07, audit SDK 0.104)** : `runAssistantTurn` déclare `tools: readonly unknown[]` puis fait `tools as unknown as Anthropic.Tool[]` ([ai-chat-routes.ts:802](packages/backend/src/api/routes/ai-chat-routes.ts#L802)). **Ce cast rendrait TypeScript aveugle à une rupture de contrat sur `Tool`** — or `tools` est le paramètre dont dépend toute l'anti-hallucination structurelle (#239). Inoffensif aujourd'hui (l'audit a prouvé que `Tool` n'a fait que **gagner** 3 champs optionnels entre 0.74 et 0.104), mais c'est un **garde-fou désarmé au point le plus sensible** : au prochain bump, on veut que le type **crie à notre place**. Fix : typer `SHOPPING_CHAT_TOOLS` en `Anthropic.Tool[]` à la source et retirer le double cast. PR dédiée. **RÉSOLU (#243)** — et l'audit a trouvé un **2ᵉ garde-fou désarmé, pire que le cast** : `ShoppingToolName` (l'union qui définit l'allowlist de chaque contexte) était **recopiée à la main**, pas dérivée. Renommer un tool dans `SHOPPING_CHAT_TOOLS` faisait que le filtre `spec.tools.includes(t.name)` ne matchait plus rien → le contexte tombait à `tools: []` → **l'assistant perdait son ancrage en silence** (et le disait honnêtement : « je n'ai pas accès aux données »). 10ᵉ instance de la classe. Fix : `SHOPPING_CHAT_TOOLS` `as const satisfies readonly Anthropic.Tool[]` (le contrat au point de déclaration) + `ShoppingToolName = (typeof SHOPPING_CHAT_TOOLS)[number]['name']` (dérivée) + `runAssistantTurn(tools: readonly Anthropic.Tool[])` + les 2 casts retirés. **Prouvé par contrôle négatif** : 3 cassures (tool sans `input_schema` / tool renommé / tableau non-Tool) → **3/3 SILENCE avant, 3/3 tsc CRIE après** (sur le renommage, tsc propose même la correction : *Did you mean 'searchCatalogV2'?*). **Acquit** : un garde-fou qu'on n'a pas heurté est une décoration — `tsc` vert ne prouve rien, il l'était déjà.
...
| **Phase 1 P2** | 🟡 En cours | **1 tâche actionnable restante** : **Backend dotenv cleanup (reclassé P3→P2 le 31/05/26 après audit, cf §12 31/05)**. Cumul résolu : 2 le 17/05 (HeroVideo + Backend 500) + 2 fermées par décision 22/05 (#3 guides hors scope + #4 TrustStack caduque alignée §8.2) + 1 résolue 23/05 (#1 HowItWorks → Card polymorphique) + **1 infra `cn()`→twMerge #235 + 1 SandboxMigrationBanner→Card+Button (dernière dette « composant » close, débloquée par #235)**. |
...
- **(d) Dette** : Lot 2-4 Dependabot ; Backend dotenv cleanup (§11 P2) ; retrait 11 vars `[LEGACY]` de `.env.example`. *(SandboxMigrationBanner→primitives + cn()→twMerge = FAITS, cf §11 P2.)*
...
*Dernière mise à jour : 12/07/2026 — **Chantier design-system : filet visuel primitives → swap cn()→twMerge → migration SandboxMigrationBanner (dette P2 « composant » CLOSE).** (1) **#234** : gallery dev-only `/dev/primitives` (route gated `VITE_DEV_GALLERY`, tree-shakée en prod) + job CI `primitives-visual` self-contained screenshotant les **13 primitives sensibles** (celles à look baké en merge racine) — 5 baselines linux générées sur runner. (2) **#235** : `cn()` `join(' )`→`twMerge` (même signature, +6,77 Ko gzip, tailwind-merge 2.6.1) → un `className` de call-site écrase enfin la classe bakée de même famille. Blast radius audité #233 = **1 seul** changement voulu (Select tri CatalogPage, `!important` retiré). Proxy local (avant/après) + `primitives-visual` linux = **0 primitive ne bouge**. (3) **cette PR** : `SandboxMigrationBanner` `<div>`→`<Card>` (**0 px**) + 3 `<button>`→`<Button>` (sous-pixel AA 787 px/1% accepté, cf **règle de non-dérive §4** : le rendu perçu — géométrie/couleur/espacement — est intact, seul l'anti-aliasing des glyphes bouge par changement de modèle de boîte `inline-flex` vs `<button>`). Test existant 3/3 **non modifié** (events A/B intacts). **Acquis** : un filet visuel dédié rend un swap d'infra risqué (twMerge) prouvable automatiquement ; §4 « étendre par props » est désormais **suffisante** (plus de verrou `cn()` additif). #233 (doc « bloqué/reporté ») **caduc** → fermé. Séquence de merges prérequis : #234 puis #235 (chacun vert sur son axe ; rouges = dette pré-existante). Branche : `feat/sandbox-banner-primitives`.*
