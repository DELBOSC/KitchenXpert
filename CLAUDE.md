...
- [ ] **CI frontend — vitest coverage-v8 crash après 1220 tests verts** (découvert 02/06/26) : le step `test:coverage` du job test-unit de frontend-ci.yml plante avec `V8CoverageProvider.convertCoverage` (provider.js:2446) **APRÈS** que les tests passent (Test Files: 45 passed, Tests: 1220 passed) → exit 1. Vitest 1.6.1 + `@vitest/coverage-v8@1.6.1`. Bug du reporter coverage-v8, **pas** le fix turbo de PR #75. À investiguer : bump coverage-v8 (PR Dependabot pending) ou config vitest. Lié à la dette security audit (la vuln high mentionne coverage-v8).
- [ ] **CI frontend + backend — ESLint rouge préexistant** (découvert 02/06/26) : `lint:frontend` (frontend-ci.yml) et `lint:backend` (backend-ci.yml) échouent au step `Run ESLint` **avant** le step type-check. Erreurs ESLint accumulées dans le code, hors scope CI. Bloque les jobs `lint` qui sont `needs` de plusieurs jobs downstream (build, etc.). À investiguer en session dédiée : `pnpm lint` localement, mesurer le volume d'erreurs et décider stratégie (fix all vs warning vs ajustement eslintrc).
- [ ] **CI security jobs rouges — pnpm audit + OWASP** (découvert 02/06/26) : les jobs `security` des 2 workflows échouent sur `pnpm audit --audit-level=high`. Vuln high dans `@vitest/coverage-v8@1.6.1` (liée à la dette vitest coverage ci-dessus). À investiguer : peut être résolu par bump coverage-v8 (PR Dependabot pending) ou exception audit ciblée.
- [ ] **🚨 False green dangereux — pnpm filter sur script absent** (découvert 02/06/26) : `pnpm --filter backend test:integration` avec le script `test:integration` ABSENT du backend package.json → pnpm filter **no-op à exit 0** → step CI **VERT sans exécuter aucun test**. Confirmé sur Backend CI run 26839748278 (job Integration Tests vert alors que rien ne tourne). Concerne `test:integration` ET `test:api` (Newman) dans backend-ci.yml. **Trompe les reviewers sur la couverture réelle.** Action : soit ajouter les scripts + leur infra (tests d'intégration réels + collection Postman), soit retirer ces jobs du workflow. Priorité moyenne (cosmétique CI mais trompeur).
- [ ] **build:backend bypass turbo** (découvert 02/06/26) : comme PR #75 côté frontend, le job `build` de backend-ci.yml (l.276) appelle `pnpm build:backend` sans passer par turbo → `@kitchenxpert/common` dist/ peut être absent au moment du `tsc` backend. Fix mécanique 1 ligne : `pnpm turbo run build --filter @kitchenxpert/backend`. Identifié comme scope OUT lors de PR #76 (préférence atomic). PR future dédiée, fix trivial.
- **🗺️ Couche 7 — suite E2E (dépilée par couches, session 06/06)**. Après couche 6 (build backend), la suite a tourné et révélé des couches successives, toutes **diagnostiquées par preuve** (logs CI + lecture code) :
  ...
  - [ ] **🆕 7f facette 3 (restant) — flows métier backend** : flow-1, flow-4 (catalog import + seeds), flow-5 (designer canvas), flow-6 (quote PDF), flow-8 (RGPD profile/export) — **chantier LOCAL** (Playwright contre stack up + seeds, PAS re-run CI). + a11y **login** `color-contrast` 1 node = **flaky** (timing de rendu, pas dur).
  - [ ] **🔬 flow-1 signup — diagnostic 09/06 (3 couches, branche `fix/e2e-flow1-signup` PR #106 WIP tenue)** :
    - **Cause A/A' (checkbox `sr-only`)** : primitive `Checkbox` = `<label htmlFor=x><input id=x class="peer sr-only">` + span visuel. `.check()` ne peut actionner l'input caché ; `.check({force})` clique mais **ne toggle pas** l'input contrôlé (prouvé local : reste false). **Fix = cliquer le span visuel** (`following-sibling::span[1]`, idempotent) → `checked=true` (prouvé local). ⚠️ Retirer le `htmlFor` redondant du primitive a été **testé local → n'y change rien** (hypothèse double-toggle infirmée) → primitive intact. **Span-click = seul contenu de #106 (WIP).**
    - **Cause B (cookie-consent intercepte le submit)** : bannière `CookieConsent` (`fixed bottom-0 z-100`, App.tsx:42) recouvre le submit `w-full` → click intercepté (prouvé CI « subtree intercepts pointer events » + repro/validé local). Dismiss via `addInitScript` dans le `page` fixture `_fixtures` corrigeait le submit (test 1 → vert) **MAIS a RÉGRESSÉ flow-2** (login→dashboard timeout, vert `b1f7cb4` → rouge `f5018ef` ×2 runs) → **reverté** (`c1686dc`). Pré-seeder le consent casse la nav navigateur post-login (mécanisme inconnu ; login **API** OK via flow-3b).
    - **Couche commune login→dashboard navigateur** : flow-1 tests 2&3 (re-login final) + flow-2 + flow-4/5/6/8 convergent sur la nav navigateur post-login (`loginUI waitForURL(/dashboard)`), instable et **sensible à l'état consent**. **Nécessite STACK-UP** (login→dashboard non reproductible sans backend). = vrai prochain blocker.
    - **Leçons** : (a) 2 hypothèses infirmées par validation locale AVANT commit (htmlFor ; puis « animation/not stable ») ; (b) « consent auth-unrelated → flake » **infirmé par comparaison de runs** (régression réelle) — ne pas conclure « flake » sans preuve ; (c) valider un fix de fixture partagé sur TOUS les flux, pas seulement la cible.
...
## 13. État d'avancement (snapshot 08/06/2026)
...
- `main` (HEAD `c6c3d3d`, post-#104 — couche 7f facette 3 login RÉSOLU : #102/#103 + visual baselines #104)
...
- `docs/session-08-06-facette3-login` (branche courante, ce doc)
...
Prochaine cible : (a) **couche 7f facette 3 restante — débogage LOCAL stack-up** (flow-1 checkbox `sr-only`, flow-4 catalog import+seeds, flow-5 designer, flow-6 PDF, flow-8 RGPD ; + catalog a11y data-driven flaky) ; (b) **infra prod §14.2** — prérequis secrets §14.1 ; (c) **Lot 2 Dependabot**. Couche 7f **facettes 1+2 + login facette 3 RÉSOLUES** (#97-#99, #102/#103) ; visual-regression baselines committées (#104, job dédié vert) ; e2e sur main = rouge informatif (flows métier backend) ; CodeQL (#2) reporté repo public.
