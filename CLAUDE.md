...
- **🗺️ Couche 7 — suite E2E (dépilée par couches, session 06/06)**. Après couche 6 (build backend), la suite a tourné et révélé des couches successives, toutes **diagnostiquées par preuve** (logs CI + lecture code) :
  ...
  - [ ] **🆕 7f facette 3 (restant) — flows métier backend** : flow-1 (checkbox `sr-only` actionability), flow-4 (catalog import + seeds), flow-5 (designer canvas), flow-6 (quote PDF), flow-8 (RGPD profile/export) — **chantier LOCAL** (Playwright contre stack up + seeds, PAS re-run CI). + a11y **login** `color-contrast` 1 node = **flaky** (timing de rendu, pas dur).
