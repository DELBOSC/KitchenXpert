...
- **(d) Dette** : Lot 2-4 Dependabot ; **chantier `cn()`→tailwind-merge (§11 P2, débloque SandboxMigrationBanner + toute migration de primitive « à look imposé »)** ; `SandboxMigrationBanner`→primitives = **BLOQUÉ par `cn()`** (§11 P2) ; Backend dotenv cleanup (§11 P2) ; retrait 11 vars `[LEGACY]` de `.env.example`.
