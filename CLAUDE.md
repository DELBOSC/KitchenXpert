...
- **🗺️ Couche 7 — suite E2E (dépilée par couches, session 06/06)**. Après couche 6 (build backend), la suite a tourné et révélé des couches successives, toutes **diagnostiquées par preuve** (logs CI + lecture code) :
  - [x] **7b — a11y `networkidle`** : RÉSOLU PR #87 (squash `62dc20c`). `networkidle` ne settle jamais (le build prod `vite preview` enregistre un **SW network-first** ; Playwright déconseille networkidle pour SPA). Fix : `#root>*` render-wait (a11y) / `load` (visual). **PROUVÉ** : axe s'exécute, **3 a11y verts** (catalog-ikea, legal-privacy, legal-cgv). Élimination code des autres pistes (LiveCounter home-only/30s, OfflineIndicator=IndexedDB, AuthContext one-shot, Plausible consent-gated, WS designer-only).
  ...
  - [ ] **🆕 7e (restant) — flux E2E UI** : run final 27058211982 = **3 passed / 25 failed**. Restant après 7a-7e : `locator.fill: Timeout 10000ms` (30 — champs login/signup non trouvés par les selectors), `toHaveURL` (6 — redirects), `401`/`Received:false` (3 — auth API), `localStorage SecurityError` (sandbox). → **chantier de débogage LOCAL** (Playwright contre stack up, itération seconde-par-seconde — PAS de re-run CI 8-12 min par couche). Constat : la suite de flux n'avait **jamais** tourné end-to-end.
  - [ ] **Dette produit a11y — `color-contrast`** : 5 pages (home/login/register/pricing/catalog) échouent axe sur `color-contrast [serious]` (< WCAG 2 AA), **même règle** → 1 fix design tokens/CSS. Surfacé une fois 7b débloqué.
  - **PR #79 (Build backend)** : toujours **ouverte/HELD** — la merger rend l'e2e ~8-12 min rouge/PR tant que 7e-UI restant + a11y ne sont pas réglés. À merger quand la suite passe (ou décision « e2e informatif »).
...
## 13. État d'avancement (snapshot 06/06/2026)
...
- `main` (HEAD `68532f8`, post-couche-7 fixes 06/06 — #86/#87/#88/#89/#90 mergées)
- `ci/e2e-build-backend` (commit `2057c0b`) — **PR #79 OUVERTE/HELD** (step Build backend e2e). À merger quand la suite E2E passe (cf couche 7 §11 P3) ou décision « e2e informatif ».
...
- `docs/session-couche7-e2e` (branche courante, ce doc)

Branches mergées et supprimées 06/06 : `fix/e2e-couche7c-preview-proxy` (#86), `fix/e2e-couche7b-networkidle` (#87), `fix/e2e-couche7d-rate-limit` (#88), `fix/e2e-couche7d-inline-auth-limiter` (#89), `fix/e2e-couche7e-verify-email-backdoor` (#90). Antérieures 02-03/06 (#75-#85) prunées également.

Prochaine cible : (a) **couche 7e restant — débogage LOCAL des flux E2E** (`locator.fill`/redirect/`401`/localStorage — Playwright contre stack up, itération rapide) ; (b) **dette a11y `color-contrast`** (5 pages, design tokens) ; (c) **infra prod §14.2** (Supabase/Upstash/hébergeur) — **prérequis aux secrets §14.1** ; (d) **Lot 2 Dependabot**. PR #79 held jusqu'à suite E2E verte ; CodeQL (#2) reporté repo public.
...
*Dernière mise à jour : 06/06/2026 — **Couche 7 E2E dépilée par couches (5 PRs : #86/#87/#88/#89/#90)**. Diagnostic 100% par preuve (logs CI + lecture code) : 7b networkidle/SW (3 a11y verts), 7c preview proxy, 7d rate-limit (2 limiters, 429=0), 7e verify-email backdoor (emailVerified Date→Boolean, verify-missing=0). Run final = **3 passed / 25 failed** : restant = **7e-UI** (`locator.fill`/redirect/`401`/localStorage → débogage LOCAL) + **dette produit a11y `color-contrast`** (5 pages). 2 corrections de cap assumées en route (no-op `ENABLE_RATE_LIMIT` ; 2ᵉ limiter raté au grep). **PR #79 (Build backend) toujours HELD** jusqu'à suite E2E verte. main HEAD = `68532f8`. Branche courante : `docs/session-couche7-e2e`.*
