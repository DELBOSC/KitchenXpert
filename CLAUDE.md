...
- [⚠️ BLOQUÉ INFRA `cn()` — 11/07/26] `SandboxMigrationBanner.tsx` : migrer vers primitives `Card`/`Button`. **Bloqué par l'infra `cn()`, prouvé à l'audit du 11/07.** [ui/_utils.ts](packages/frontend/src/components/ui/_utils.ts) `cn()` est un simple `join(' ')` **sans résolution de conflits** (le fichier le documente lui-même : « Not as powerful as clsx+tailwind-merge… swap in tailwind-merge if we need conflict resolution »). Conséquence : `Card` impose **toujours** `rounded-2xl` + `border border-white/10` + `bg-white/[0.03]` (même `glass`) ; `Button` bake `rounded-full` + halo `shadow-[…40px…]` (primary) + `h-8` + framer-motion (`whileHover:{y:-1}`, `whileTap:{scale:.97}`). Un `className` d'override **s'additionne au lieu de remplacer** → la classe primitive ET la mienne coexistent → c'est l'**ordre-source du CSS** (pas l'ordre du string) qui tranche → **drift visuel imprévisible** (radius, border-color, tint bg parasite, blur), et le lift framer-motion est **inamovible via className**. Donc « migrer vers primitives » + « zéro dérive visuelle » = **incompatibles en l'état**. La bannière porte le tracking A/B `sandbox_signup_completed_ab` (import OU delete) → un changement d'apparence pollue l'expérience en cours → **A (adopter le look DS) refusé** (décision Laurent 11/07 : pas comme effet de bord d'un nettoyage). **⚠️ Piège à NE PAS refaire (audit 11/07)** : « sortir l'apparence (rounded/height) du merge racine de Button vers les variants » **NE débloque PAS** sous `cn()` additif — déplacer une classe base→variant **ne la retire pas de la sortie concaténée** ; `rounded-full` en `sizes[size]` sort toujours, et un call-site `className="rounded-lg"` produit `rounded-full … rounded-lg` → l'ordre-source tranche (`rounded-full` gagne, généré après). Seuls **twMerge**, **`!important`**, ou **l'absence totale de la famille** dans la primitive rendent un override fiable → **ni Card ni Button ne se contournent proprement aujourd'hui**. **Débloqué par →** le chantier `cn()`→tailwind-merge (ci-dessous, **REPORTÉE** — voir la vraie raison). Dette **bloquée-et-tracée** : elle se rouvre mécaniquement le jour où `cn()` résout les conflits. Branche `feat/sandbox-migration-banner-primitives` (vide) supprimée. NB probable : c'est vraisemblablement **cette même contrainte** qui a fait écrire la bannière from-scratch à l'origine — pas de la paresse.
- [⚠️ REPORTÉE — 11/07/26] **Chantier `cn()` → tailwind-merge** : swapper [ui/_utils.ts](packages/frontend/src/components/ui/_utils.ts) `cn()` (naive `join`) pour `twMerge(parts.filter(Boolean).join(' '))` **derrière la même signature** (le fichier l'anticipe ; **clsx inutile** — la signature variadique filtre déjà les falsy). **Audit exhaustif du blast radius le 11/07 → REPORTÉ, PAS un chantier sûr à faire maintenant.** Chiffres RÉELS (inventaire de tous les `className` passés aux 16 primitives — string `="…"` + expression `={…}` + spread + conflits internes) : **1 SEUL conflit** — [CatalogPage.tsx:350](packages/frontend/src/pages/CatalogPage.tsx#L350) `<Select className="!h-9 w-auto">` (le `!h-9` gagne **déjà** via `!important` → inchangé ; `w-auto` perd aujourd'hui contre `w-full` baké — ordre Tailwind — et **gagnerait** sous twMerge → le select de tri **rétrécit**), catégorie **(a)** = override voulu inopérant que twMerge **répare** (changement vers l'intention). **ZÉRO** override mort catégorie (b) ; **zéro** conflit interne aux primitives (les ternaires sont mutuellement exclusifs, `px-4`+`pl-10` se résout à l'identique). Donc le swap = **1 changement visuel intentionnel**, PAS une régression de masse. **Pourquoi reporté malgré ça** : le **filet automatique est quasi nul** — 5 baselines Playwright (#104 : home/login/dashboard/catalog/designer), **aucune sur Button/Card/Input**, et l'E2E est **déjà rouge en dette** ; la garantie « 1 conflit » repose sur un **inventaire manuel** (« confiance haute, pas absolue »). Sans baselines sur les primitives, un angle mort d'inventaire se découvrirait **en prod**, une régression à la fois. **Rouvrir quand → baselines visuelles Button/Card/Input en place** (condition unique). **Coût si fait** : **+~7 Ko gzip** (tailwind-merge **v2.x** — v3 = Tailwind v4, **incompatible** avec `tailwindcss ^3.4.1`), dépendance net-new (contre l'actuel « zero-dependency »). **Bénéfice si fait** : débloque **Card ET Button** (leurs looks — `rounded-2xl`/border/bg pour Card, `rounded-full`/halo/`h-8` pour Button — sont dans le **merge racine**, donc overridables une fois twMerge en place) + toute future migration de primitive « à look imposé ». **Leçon §4 + corollaire (à garder)** : merge racine = structurel non-négociable, variants = apparence — **règle nécessaire mais NON suffisante** : tant que `cn()` est additif, une primitive qui met son look en racine (Card : `rounded-2xl`+border+bg) reste **non-overridable même avec un découpage propre**. Seuls **twMerge**, **`!important`**, ou **l'absence totale de la famille** rendent un override fiable. §4 est une règle de **conception** des primitives ; `cn()`-additif est un **second verrou d'infra** — les deux doivent tomber pour que « étendre par props » marche vraiment. (Correction d'un raccourci antérieur : §4 n'est pas « structurellement inapplicable » à cause de `cn()` seul — elle est nécessaire, et `cn()` est le verrou complémentaire.)
...
| **Phase 1 P2** | 🟡 En cours | **1 tâche actionnable restante** : **Backend dotenv cleanup (reclassé P3→P2 le 31/05/26 après audit, cf §12 31/05)**. + **1 bloquée** : SandboxMigrationBanner→primitives (⚠️ BLOQUÉ INFRA `cn()` 11/07, débloqué par le chantier `cn()`→tailwind-merge). + **1 REPORTÉE** : chantier `cn()`→tailwind-merge (audit 11/07 : **1 seul conflit** mesuré, mais filet visuel quasi nul — aucune baseline Button/Card/Input, E2E rouge → reporté jusqu'à baselines). Cumul résolu : 2 le 17/05 (HeroVideo + Backend 500) + 2 fermées par décision 22/05 (#3 guides hors scope + #4 TrustStack caduque alignée §8.2) + 1 résolue 23/05 (#1 HowItWorks → Card polymorphique, commits 594c63b+ee1869c). |
...
- **(d) Dette** : Lot 2-4 Dependabot ; chantier `cn()`→tailwind-merge = **REPORTÉ** (§11 P2 ; 1 conflit mesuré mais 0 baseline primitive → rouvrir quand baselines Button/Card/Input) ; `SandboxMigrationBanner`→primitives = **BLOQUÉ par `cn()`-additif** (§11 P2) ; Backend dotenv cleanup (§11 P2) ; retrait 11 vars `[LEGACY]` de `.env.example`.
