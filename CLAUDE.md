...
- [⚠️ EN COURS — Group A fait 10/06] **Triage vulns transitives (hors bloc override historique)** : les 43 vulns restantes après l'audit override, triées par preuve (`pnpm audit --json` + `pnpm why`). **Group A ✅ FIXÉ (#127)** : 8 advisories à override same-major patché → handlebars `^4.7.9` (critical), lodash `^4.18.0` (high runtime/recharts), rollup `^4.59.0`, flatted `^3.4.0`, tmp `^0.2.6` (high runtime/exceljs), + **scopés par version** minimatch@9 `^9.0.9`, picomatch@2 `^2.3.2`, picomatch@4 `^4.0.4` (le scope évite de re-casser eslint@8 = piège #116). **Résultat 43→21.** **Group B ⏳ (différé, dev-only + lourd)** : **vitest 1→3** (critical UI-server dev-only ; bump majeur test-infra 1226 tests — coordonner avec §14.5 Lot 4) + **storybook 8.6.15→8.6.17+** (high dev-server WebSocket ; skew meta-package, aligner tous les `@storybook/*` via Dependabot). **Group C ⏳** : 14 moderate + 5 low (esbuild, qs, ajv, vite, postcss, astro, ip-address, ws, turbo, uuid, react-router, diff, @tootallnate/once) — certains runtime (ws, react-router, qs, uuid), beaucoup fixables par override same-major, à batcher. Technique clé : **override scopé `pkg@major`** quand une seule ligne de version est vulnérable.
