...
**Pendings P7.** (1) **Validation conversationnelle** via le frontend (route `POST /api/v1/ai-chat/shopping` authentifiée — tester en UI, pas en curl) : vérifier dans les logs que le LLM appelle `resolve_colors` et ne propose que les couleurs retournées. (2) UI color-picker (greenfield, mount `ChatPanel`). (3) Brancher `bom-generator` sur les vrais SKU résolus (dette §15.7 P4 : fin du `catalogRef` halluciné).
