...
- [⚠️ EN COURS — audit fait 10/06, 1/3 fixé] **Audit `pnpm.overrides`** : bloc racine audité entièrement le 10/06 (10 entrées). **3 catégories** : (a) 🔴 **jspdf `^3.0.0` toxique — FIXÉ PR #118** : forçait jspdf en 3.0.4 (CRITICAL path-traversal `<=3.0.4`) alors que code+dep frontend veulent v4 ; override périmé (ère-v3, jamais MAJ au bump v4 Lot 1 #28) → retiré, résout 4.2.1 (patché, clôt aussi `<=4.2.0`). **4ᵉ override toxique du commit `edb726b`** (après path-to-regexp #47, minimatch #116). (b) 🟠 **pins CVE inadéquats → #5 security** : `nodemailer ^7.0.4` (encore `<8.0.4` vuln ; backend déclare v6 ; bump v8 = 2 majeures breaking), `axios ^1.12.0` (résout 1.13.2, encore `<1.15.2` vuln — pin inefficace). (c) ✅ **7 pins légitimes** (dompurify, tar, fast-xml-parser, undici, basic-ftp, protobufjs, follow-redirects) — sécu transitifs, aucun forçage de majeure, **à re-vérifier qu'ils pointent une version patchée** en #5. Règle confirmée : un override doit avoir (a) un consommateur réel, (b) une raison CVE documentée, (c) **pointer une version RÉELLEMENT patchée** (pas un pin périmé/inefficace).
