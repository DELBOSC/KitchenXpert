...
- [ ] **`aiUnauthRateLimiter` plafonne les users AUTHENTIFIÉS à 5/h sur `/ai-chat` (ordre des middlewares)** — trouvé 28/06 (validation chatbot couleur). Le mount `router.use('/ai-chat', aiUnauthRateLimiter, aiChatRoutes)` ([routes/index.ts:120](packages/backend/src/api/routes/index.ts#L120)) exécute `aiUnauthRateLimiter` (5/h par IP) **avant** `authenticate` (par-route) → `req.user` est `undefined` au mount → son `skip(req => Boolean(req.user?.userId))` ne se déclenche **jamais** → les requêtes authentifiées sont comptées par la limite « unauth » 5/h, au lieu des 20/h de `aiRateLimiter`. **Prouvé** : 429 `RATE_LIMIT_EXCEEDED` au 6ᵉ appel authentifié sur `/ai-chat/shopping` (contourné en validation par `NODE_ENV=test` qui skip les limiters). Bug latent réel (sous-quota silencieux pour les users connectés). **Fix = chantier de code séparé** : réordonner/fusionner les limiters `/ai-chat` (faire passer `authenticate` avant le limiter conditionnel, ou un seul limiter avec `keyGenerator` user-aware) ; **preuve de fix = le 6ᵉ appel authentifié passe**. Non traité ici (doc-only).
