...
## 13. État d'avancement (snapshot 02/06/2026)
...
| **Phase 1 P3** | 🟡 Entamée (cluster CI 02/06) | **~27 tâches actionnables internes** (recompte 02/06 — la ligne "20" du 31/05 matin n'intégrait pas les 7 dettes ajoutées le 31/05 soir → baseline réelle ≈ 27). Cette session : **−2 résolues** (#3 frontend-ci PR #75, #4 backend-ci PR #76), **−1 résolue** (#5 submodule PR #77), **−1 caduque** (#7 indent e2e), **−1 reclassée BLOQUÉ EXTERNE** (#2 CodeQL Advanced Security), **+5 nouvelles 02/06** (vitest coverage-v8 crash, ESLint préexistant front+back, security audit, 🚨 false green pnpm filter, build:backend turbo) = **27 net**. |
...
- `main` (HEAD `c512172`, post-cluster-cleanup CI 02/06 — PRs #75/#76/#77 mergées)
...
- `docs/session-02-06-cluster-cleanup-ci` (branche courante, ce doc)

Branches mergées et supprimées cette session (refs prunés au merge `--delete-branch`) : `ci/turbo-frontend-ci` (#75), `ci/backend-ci-scripts-and-db-env` (#76), `chore/remove-orphan-submodule` (#77). Antérieures (31/05) : `chore/lockfile-sync-pg`, `docs/session-31-05-pg-dotenv-audit`, `ci/fix-pnpm-setup-order-e2e-lighthouse`, `ci/fix-prisma-schema-e2e`, `ci/turbo-frontend-build-deps`, `ci/migrate-labeler-v5-config`.

Prochaine cible : (a) **couche 6 backend startup E2E** (cf §11 P3) — wait-on timeout, probable env vars manquantes Stripe/Anthropic/AWS au boot ; (b) **Lot 2 Dependabot** (Actions + dotenv) débloqué (facturation OK), PR #15 dotenv prouvée safe ; (c) **dettes CI 02/06** (vitest coverage-v8, ESLint front+back, security audit, 🚨 false green pnpm filter, build:backend turbo) ; (d) **§14.1 sécurité secrets** — bloquant prod. CodeQL (#2) reporté au passage repo public.
...
*Dernière mise à jour : 02/06/2026 — **Cluster Cleanup CI : 3 PRs CI mergées (#75 frontend-ci turbo, #76 backend-ci DB provisioning, #77 submodule orphelin) + CodeQL reclassé (Advanced Security required) + 4 nouvelles dettes cascade découvertes (vitest cov, ESLint préexistant, security audit, false green pnpm filter)**. Dettes §11 P3 résolues : #3 frontend-ci TS6305, #4 backend-ci provisioning, #5 submodule orphelin, #7 indent YAML caduque. CodeQL #2 reclassé "bloqué par Advanced Security repo privé, reporter post-public". Pattern validation post-merge avec preuve `gh run view` + grep logs adopté comme norme. main HEAD = `c512172`. Branche courante : `docs/session-02-06-cluster-cleanup-ci`.*
