...
- **🗺️ Couche 7 — suite E2E (dépilée par couches, session 06/06)**. Après couche 6 (build backend), la suite a tourné et révélé des couches successives, toutes **diagnostiquées par preuve** (logs CI + lecture code) :
  ...
  - [ ] **🆕 7f facette 3 (restant) — login 401 / backend** : `waitForURL(/dashboard/)` timeout sur flow-1/2/4/5/6/7/8 + flow-3b migration (~12 tests) = **auth/seed/cookie en preview**, **chantier LOCAL** (Playwright contre stack up, PAS re-run CI). + visual-regression ×5 = **baselines non committées** (infra). + a11y **login** `color-contrast` 1 node = **flaky** (passe au retry, timing de rendu, pas dur).
  - [ ] **🆕 Catalog color-contrast data-driven (révélé #99, run 27098689257, 07/06)** : a11y `catalog` (/catalog ProvidersHub) = **hard fail, 4-5 nodes `color-contrast` [serious]** en CI (DB seedée), alors que mon scan headless local **sans backend** affichait **0** sur cette même page. Cause : les **cartes fournisseurs peuplées** (données seedées) exposent du texte basse-contraste qu'un scan sans backend ne rend pas. À traiter avec **facette 3** (stack up requis pour reproduire/corriger).
...
## 13. État d'avancement (snapshot 07/06/2026)
...
- `main` (HEAD `d89ff92`, post-#100 — couche 7f facettes 1+2 : #97/#98/#99 mergés + doc #100)
...
- `docs/claude-md-catalog-data-driven-debt` (branche courante, ce doc — trace la dette catalog data-driven révélée par #99)
...
Prochaine cible : (a) **couche 7f facette 3 — débogage LOCAL** (login 401→dashboard ~12 flows + flow-3b migration ; auth/seed/cookie en preview ; Playwright contre stack up) + **catalog color-contrast data-driven** (4-5 nodes, besoin backend, cf §11 P3) + visual-regression baselines à committer + a11y login flaky ; (b) **infra prod §14.2** — prérequis secrets §14.1 ; (c) **Lot 2 Dependabot**. Couche 7f **facettes 1+2 RÉSOLUES** (#97/#98/#99) ; e2e sur main = rouge informatif (facette 3) ; CodeQL (#2) reporté repo public.
