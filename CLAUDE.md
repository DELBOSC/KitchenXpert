...
- [ ] **Verdir le job "Lint & Type Check" backend** : 19 erreurs ESLint (17 import/order), **toutes auto-fixables** (`eslint --fix`) MAIS à relire pour effets de bord (un réordonnancement peut casser un pattern type `dotenv config()` qui doit rester après les imports — leçon P5). + config Prettier frontend cassée (`packages/frontend/.prettierrc.js` traité ESM -> renommer `.cjs`). Le tsc est découplé (#191) donc cette dette ne masque plus les erreurs de type ; rouge honnête/visible. Chantier dédié.
...
## 13. État d'avancement (snapshot 23/06/2026)
