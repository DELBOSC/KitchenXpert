...
- [ ] Backend dotenv cleanup : retirer les 3 `dotenv.config()` redondants (`packages/backend/src/config/app-config.ts:3` + `packages/backend/src/config/env-validator.ts:20-21`) une fois `load-env.ts` validé en place. `load-env.ts` devient la source unique de chargement du `.env` racine. ⚠️ **PAS purement cosmétique** — confirmé par audit 31/05/26 (cf §12 31/05) : ces appels ne sont PAS idempotents en pratique parce qu'ils mitigent un problème d'**ordre d'imports** (auth.service tiré avant env-validator, jwt.service lit `process.env` au module-eval). Un cleanup naïf des 3 appels casse le boot HTTP : le backend démarre, les logs sont propres, mais Express ne sert plus `/api/v1/*` (browser voit `ERR_CONNECTION_REFUSED`). Branche de repro `chore/dotenv-17-minimal` (commit a10f4aa) prouve que c'est le cleanup qui casse, **pas** le bump dotenv 17. Avant de retirer les appels : (a) auditer l'ordre d'imports complet via `tsc --listFiles` ou trace require, (b) déplacer `load-env.ts` en tout premier import du module entry, (c) tester `pnpm dev` + smoke browser sur `http://localhost:3005`, pas seulement curl `/health`.
...
- **31/05/2026** : **Audit pg lockfile drift + repro minimale dotenv 17 (audit-first, deux branches dédiées).**
...
| **Phase 1 P3** | ⏳ Non démarrée | **21 tâches actionnables** (–1 par décochage `bug prisma stats` cumulé via PR #46, +1 par ajout 31/05 "Lockfile drift après merge Dependabot"). Détail historique : 8 originales + 4 ajoutées 22/05 + 1 ajoutée 23/05 backend dotenv (mise à jour 31/05 : note "PAS cosmétique") + 5 ajoutées 23/05 Redis prod-grade + 2 ajoutées 27/05 matin (bug prisma résolu + hygiène 36 branches) + 1 ajoutée 27/05 soir (audit `pnpm.overrides`) + 1 ajoutée 31/05 (lockfile drift Dependabot). |
...
| **§14.5 Chantier Dependabot** | 🟢 Lot 1 complet | **Lot 1 ✅ 11/11 (27/05 fin de soirée).** Lot 2 amorcé indirectement par audit 31/05 : PR #15 (dotenv 16→17) prouvée safe en isolat (cf §12 31/05) — peut être mergée après `chore/lockfile-sync-pg`. Reste : 5 PRs Lot 2 (Actions + dotenv déjà audité) + 5 PRs Lot 3 + 8 PRs Lot 4. |

**Branches actives** :

- `main` (HEAD `8d311eb`, post-merge `docs/lot1-complete` via PR #49)
- `chore/lockfile-sync-pg` (commit `2218c2a`) — atomic 1-fichier, débloquera CI `--frozen-lockfile`, **safe à merger immédiatement**
- `chore/dotenv-17-minimal` (commit `a10f4aa`) — artefact d'audit, **NE PAS merger** (laisser Dependabot rebaser PR #15)
- `docs/session-31-05-pg-dotenv-audit` (branche courante, doc CLAUDE.md)

Prochaine cible : merge `chore/lockfile-sync-pg` + cette branche doc → main, puis laisser PR #15 Dependabot rebaser et la merger. En parallèle : démarrer §14.1 sécurité secrets dès que possible (priorité prod).
...
*Dernière mise à jour : 31/05/2026 — **Audit pg lockfile drift + repro minimale dotenv 17**. Deux branches dédiées créées (audit-first, atomic commits) : `chore/lockfile-sync-pg` (commit 2218c2a, sync pg `8.16.3 → 8.21.0` pour matcher `^8.18.0` du package.json — PR #21 avait perdu sa co-modif lockfile au rebase) et `chore/dotenv-17-minimal` (commit a10f4aa, repro stricte PR #15 prouvant que dotenv 17 seul ≠ régression). Conclusion : le bug `ERR_CONNECTION_REFUSED` observé sur la PR manuelle `dotenv-17-and-lockfile-sync` vient du cleanup des 3 `dotenv.config()` vestigiaux (mitigation d'un problème d'ordre d'imports), PAS du bump dotenv 17. Nouvelle dette §11 P3 ajoutée : règle process "vérifier lockfile après merge Dependabot". §11 P3 "bug Prisma stats" décoché (résolu PR #46, re-confirmé live). Branche courante : `docs/session-31-05-pg-dotenv-audit`.*
