...
- [ ] **🆕 Couche 7b — suite E2E 29/29 flux KO systémique** (découvert 02-03/06/26, après résolution couche 6 via #79+#80+#81) : run 26843210869 — la suite Playwright charge enfin et exécute les 29 tests → **TOUS échouent** (84 ✘ avec retries, jusqu'au timeout 25 min du job). Signature **a11y** (8 pages × 3 retries, ~22s chacun) : `TimeoutError: page.waitForLoadState: Timeout 20000ms exceeded` sur `page.goto(url)` (`accessibility.spec.ts:32`). **Forte suspicion** : `networkidle` jamais atteint à cause d'un polling permanent côté front (piste **LiveCounter `/api/v1/stats/public`** ou **WebSocket collab** — à confirmer). Flux fonctionnels (signup/login/logout/sandbox) : échecs d'assertion rapides (0.4-12s). C'est un **chantier dédié** : harness E2E + comportement app + attentes specs. Non triviable dans une session standard — **estimation 4-8h** selon profondeur. **PR #79 ouverte/held jusqu'à résolution** (la merger rendrait l'e2e rouge ~25 min sur chaque PR sans la rendre verte).
...
## 13. État d'avancement (snapshot 03/06/2026)
...
- `main` (HEAD `ed5d90f`, post-#82 doc couche6-7b ; #80/#81 mergées 02-03/06)
- `ci/e2e-build-backend` (commit `b919c1b`) — **PR #79 OUVERTE/HELD** (step Build backend e2e). À merger quand couche 7b sera réglée (cf §11 P3).
- **PRs cleanup §14.1 OUVERTES (03/06, non mergées)** : **#83** (`chore/remove-dead-redis-config` — retrait code mort `REDIS_*`), **#84** (`docs/env-production-example-dbvars-redis` — `DB_*` + criticité Redis dans le template prod), **`docs/claude-md-section-14-1-cleanup`** (ce doc).
- `origin/chore/dotenv-17-minimal` (commit `a10f4aa`, **remote-only** — local supprimé) — artefact d'audit, **NE PAS merger** (laisser Dependabot rebaser PR #15).

Branches mergées et supprimées 02-03/06 (refs prunés au `--delete-branch`) : `chore/align-aws-sdk-presigner` (#80), `chore/add-axe-core-playwright` (#81), `ci/turbo-frontend-ci` (#75), `ci/backend-ci-scripts-and-db-env` (#76), `chore/remove-orphan-submodule` (#77), `docs/session-02-06-cluster-cleanup-ci` (#78), `docs/session-02-06-couche6-7b` (#82). Antérieures 31/05 prunées également.

Prochaine cible : (a) merger les 3 PRs cleanup §14.1 (#83, #84, ce doc) puis cocher/décocher en conséquence ; (b) **couche 7b — chantier E2E dédié** (29/29 flux KO, `waitForLoadState`/networkidle → polling LiveCounter/WebSocket ; ~4-8h) ; (c) **infra prod §14.2** (Supabase/Upstash/hébergeur) — **prérequis aux secrets §14.1** ; (d) **Lot 2 Dependabot**. Couche 6 résolue (#79 held jusqu'à 7b) ; CodeQL (#2) reporté repo public.
...
*Dernière mise à jour : 03/06/2026 — **Cleanup §14.1 sécurité secrets (audit + 3 PRs)**. Audit READ-ONLY : git history **propre** (0 vrai secret leaké — les matches sont des générateurs `openssl`, templates, `sk_test_` en doc). Le cloud `.env` actuel (Supabase `cllgrpeepnxripgdkbba` + Upstash `giving-bird-134810`) est **dev/staging**, pas un prod distinct (§14.2 tout `[ ]`). 3 PRs ouvertes : **#83** (retrait code mort `REDIS_HOST/PORT/PASSWORD` — le singleton lit `REDIS_URL`), **#84** (template prod : + bloc `DB_*` pg-natif boot-bloquant, Redis `REQUIRED→RECOMMENDED`), **ce doc** (§14.1 : `DATA_ENCRYPTION_KEY` format libre scrypt + note `DB_*` prod-bloquants + prérequis infra non créée ; nouvelle dette §11 P3 « double-source DB env »). Corrections fondées sur le code (crypto.ts scrypt, connection.ts pg natif, redis-client.ts). main HEAD = `ed5d90f`. Branche courante : `docs/claude-md-section-14-1-cleanup`.*
