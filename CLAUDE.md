...
- [ ] **Cleanup `config/linters/` (3 orphelins restants)** : `eslintrc.js`, `prettierrc.js`, `commitlintrc.js` sont du scaffolding mort du commit initial (21/02) — parsables mais **inutilisés** (0 référence fonctionnelle ; les vraies configs vivent à la racine + `packages/*`). À supprimer (dead-code, pas d'urgence de parse, contrairement à `stylelintrc.js` qui était cassé et déjà retiré #210). `CONFIG_ANALYSIS.md` les marque déjà ❌. Vérif par fichier avant `git rm` (grep 0 réf, comme pour stylelintrc). Le dossier `config/linters/` entier disparaîtra alors.
