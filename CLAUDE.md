...
- **(a) Catalogue/IA** : valider la **conversation frontend** du chatbot couleur (route authentifiée, tester en UI) ; **UI color-picker** (greenfield, mount `ChatPanel`) ; **re-run 6 cats Castorama manquantes** (§15.8.3 P1, plafond 300-500/cat) + audit qualité post-complet (§15.8.2) ; **P8 rendu 3D dynamique**. **BOM-b reste BLOQUÉ** par prérequis data (§15.8.8 — chantier d'ingestion familles sol/crédence/quincaillerie d'abord).
