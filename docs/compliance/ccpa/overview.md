# CCPA Compliance Overview

**Last Updated:** 2026-01-10

## Table of Contents

1. [Introduction](#introduction)
2. [What is CCPA](#what-is-ccpa)
3. [Applicability to KitchenXpert](#applicability-to-kitchenxpert)
4. [Key Definitions](#key-definitions)
5. [Consumer Rights Summary](#consumer-rights-summary)
6. [Do Not Sell My Personal Information](#do-not-sell-my-personal-information)
7. [Non-Discrimination Principle](#non-discrimination-principle)
8. [Related Documentation](#related-documentation)

---

## Introduction

The California Consumer Privacy Act (CCPA) provides California residents with
specific rights regarding their personal information. KitchenXpert is committed
to CCPA compliance.

---

## What is CCPA

CCPA gives California residents rights to know, delete, opt-out of sale, access,
correct, and non-discrimination regarding their personal information.

---

## Applicability to KitchenXpert

KitchenXpert meets CCPA threshold requirements (annual revenue over M) and must
comply.

---

## Key Definitions

**Personal Information**: Identifiers, customer records, commercial info,
internet activity, geolocation, inferences

**Consumer**: California resident

**Sale**: Disclosure for monetary or other valuable consideration

---

## Consumer Rights Summary

1. **Right to Know**: Categories and specific pieces of personal information
2. **Right to Delete**: Request deletion (with exceptions)
3. **Right to Opt-Out**: Opt out of sale of personal information
4. **Right to Non-Discrimination**: Equal service regardless of rights exercised
5. **Right to Correct**: Request correction of inaccurate data
6. **Right to Limit Sensitive PI**: Limit use of sensitive personal information

---

## Do Not Sell My Personal Information

**Link**: https://kitchenxpert.com/privacy/do-not-sell

KitchenXpert does not sell personal information traditionally. Opt-out disables
advertising cookies, affiliate tracking, and third-party pixels.

---

## Non-Discrimination Principle

KitchenXpert does not discriminate against consumers who exercise CCPA rights.

---

## Related Documentation

- [Consumer Rights Under CCPA](./consumer-rights.md)
- [Opt-Out Mechanism](./opt-out-mechanism.md)
- [GDPR Overview](../gdpr/overview.md)
- [Privacy Policy](../../legal/privacy-policy.md)

---

## Document Control

| Property       | Value                   |
| -------------- | ----------------------- |
| Document Owner | Data Protection Officer |
| Last Reviewed  | 2026-01-10              |
| Version        | 2.0                     |

---

_For CCPA questions, contact privacy@kitchenxpert.com._
