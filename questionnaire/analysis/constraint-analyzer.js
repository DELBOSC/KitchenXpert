...
const CONSTRAINT_CATEGORIES = {
  ...
  spatial: {
    ...
    type: 'physical',
    flexibility: 'low',
    impacts: ['layout-options', 'appliance-sizes', 'storage-capacity', 'traffic-flow', 'work-triangle'],
    dependencies: ['structural', 'budget'],
    mitigationStrategies: ['creative-layout', 'compact-appliances', 'vertical-storage', 'multi-function-zones'],
    description: {
  ...
  lifestyle: {
    ...
    type: 'preference',
    flexibility: 'high',
    impacts: ['feature-priorities', 'durability-requirements', 'layout-preferences', 'appliance-selection'],
 ...
 */
const BUDGET_LEVELS = {
  'under-10k': { level: 1, tier: 'entry', maxQuality: 'value', limitations: ['scope', 'materials', 'appliances'] },
  '10k-25k': { level: 2, tier: 'budget', maxQuality: 'value-plus', limitations: ['scope', 'premium-materials'] },
...
function analyzeConstraints(responses) {
  ...
  analysis.resolutionPlan = generateResolutionPlan(analysis.conflicts, analysis.constraints, responses);
...
function collectBudgetConstraints(responses, constraints) {
  ...
  if (budgetAnswers['total-budget']) {
    ...
    constraints.push({
      ...
      implications: {
        en: budgetInfo.limitations.length > 0
        ...
        fr: budgetInfo.limitations.length > 0
  ...
  if (budgetAnswers['contingency-comfort'] === 'no-buffer') {
    constraints.push({
      ...
      riskFactor: 1.5,
      recommendations: {
        en: ['Consider allocating at least 10% for unexpected costs', 'Prioritize known-cost items to minimize surprises'],
        fr: ['Envisagez d\'allouer au moins 10% pour les coûts imprévus', 'Priorisez les articles à coût connu pour minimiser les surprises']
      }
    });
  }

  if (budgetAnswers['financing-preference']) {
    const needsFinancing = ['loan', 'heloc', 'credit', 'payment-plan'].includes(budgetAnswers['financing-preference']);
    if (needsFinancing) {
      constraints.push({
        id: 'financing-needed',
        category: 'budget',
...
function collectSpatialConstraints(responses, constraints) {
  ...
  if (spatialAnswers['ceiling-height']) {
    const isLow = spatialAnswers['ceiling-height'] === 'low' || spatialAnswers['ceiling-height'] === 'standard';
    if (isLow) {
      constraints.push({
        id: 'ceiling-height',
        category: 'spatial',
  ...
  if (spatialAnswers['window-placement'] === 'multiple' || spatialAnswers['window-placement'] === 'limited') {
    constraints.push({
...
function collectRegulatoryConstraints(responses, constraints) {
  ...
  if (['full-gut', 'major-remodel', 'structural-changes'].includes(futureAnswers['renovation-scope'])) {
    constraints.push({
      id: 'permits-required',
      category: 'regulatory',
      type: 'hard',
...
function collectTimelineConstraints(responses, constraints) {
  ...
  if (futureAnswers['timeline']) {
    ...
    constraints.push({
      id: 'timeline',
      category: 'timeline',
      type: timelineInfo.severity === 'critical' || timelineInfo.severity === 'high' ? 'hard' : 'soft',
...
function collectLifestyleConstraints(responses, constraints) {
  ...
  if (cookingAnswers['cooking-frequency'] === 'daily-extensive') {
    constraints.push({
      ...
      requirements: ['durable-countertops', 'high-cfm-hood', 'ample-prep-area', 'quality-appliances']
    });
  }

  // Professional cooking
  if (cookingAnswers['cooking-level'] === 'professional' || cookingAnswers['cooking-level'] === 'advanced') {
    constraints.push({
  ...
  if (socialAnswers['entertaining-frequency'] === 'frequently' || socialAnswers['entertaining-frequency'] === 'very-frequently') {
    constraints.push({
  ...
  if (socialAnswers['household-composition']?.includes('children') || socialAnswers['children-ages']) {
    constraints.push({
  ...
  // Multiple cooks
  if (cookingAnswers['multiple-cooks'] === 'yes' || cookingAnswers['multiple-cooks'] === 'frequently') {
    constraints.push({
      id: 'multiple-cooks',
      category: 'lifestyle',
      type: 'soft',
...
function collectEnvironmentalConstraints(responses, constraints) {
  ...
  if (envAnswers['eco-priority'] === 'very-important' || envAnswers['eco-priority'] === 'essential') {
    constraints.push({
      id: 'eco-priority',
      ...
      requirements: ['energy-star-appliances', 'sustainable-materials', 'low-voc-finishes', 'water-efficient-fixtures']
    });
  }
...
function collectAccessibilityConstraints(responses, constraints) {
  ...
  if (accessAnswers['accessibility-required'] === 'yes' || accessAnswers['ada-compliance']) {
    constraints.push({
      ...
      requirements: ['ada-compliant-heights', 'wide-aisles', 'accessible-appliances', 'lever-handles']
    });
  }
  ...
  if (accessAnswers['wheelchair-user']) {
    constraints.push({
      ...
      requirements: ['60-inch-turning-radius', 'lowered-counters', 'knee-space', 'side-opening-appliances']
    });
  }
  ...
  if (accessAnswers['aging-in-place']) {
    constraints.push({
      ...
      requirements: ['universal-design', 'varied-counter-heights', 'good-lighting', 'easy-grip-hardware']
    });
  }
  ...
  if (aesthetic['countertop-material']) {
    ...
    if (materialTier > budgetLevel + 1) {
      conflicts.push({
        ...
        resolution: {
          en: getAlternativeRecommendation('countertops', aesthetic['countertop-material'], budgetLevel),
          fr: getAlternativeRecommendation('countertops', aesthetic['countertop-material'], budgetLevel, 'fr')
        }
      });
    }
  }
...
function identifySpaceConflicts(constraints, responses, conflicts) {
  ...
  if (spatialAnswers['island-preference']) {
    const wantsIsland = ['large-island', 'medium-island', 'island-seating'].includes(spatialAnswers['island-preference']);
  ...
  if (spatialAnswers['preferred-layout']) {
    ...
    if (!sizeInfo.feasibleLayouts.includes(preferredLayout) && !sizeInfo.feasibleLayouts.includes('all')) {
      conflicts.push({
...
function identifyAccessibilityConflicts(constraints, responses, conflicts) {
  ...
  if (accessConstraints.length > 0 && sizeConstraint) {
    ...
    if (sizeInfo.storageCapacity === 'limited') {
      const aisleRequirement = constraints.find(c =>
        c.requirements && c.requirements.includes('wide-aisles')
      );
...
function prioritizeConstraints(constraints, conflicts) {
  return constraints.map(constraint => {
    ...
    const relatedConflicts = conflicts.filter(c =>
      c.constraints.includes(constraint.id) ||
      c.preference === constraint.id
    );
...
function assessRisks(analysis) {
  ...
  if (noContingency) {
    ...
    risks.factors.push({
      ...
      impact: { en: 'May need to reduce scope if issues arise', fr: 'Peut nécessiter de réduire la portée si des problèmes surviennent' }
    });
  }
  ...
  if (timelineConflicts.length > 0) {
    ...
    risks.factors.push({
      ...
      impact: { en: 'Project completion deadline at risk', fr: 'Date limite d\'achèvement du projet à risque' }
    });
    ...
    risks.factors.push({
      ...
      impact: { en: 'Limited buffer for unexpected delays', fr: 'Marge limitée pour les retards imprévus' }
    });
  }
  ...
  if (structuralConstraints.length > 3) {
    ...
    risks.factors.push({
      ...
      impact: { en: 'Design options significantly limited', fr: 'Options de design significativement limitées' }
    });
  }

  // Quality risk assessment
  const budgetExpectationConflicts = analysis.conflicts.filter(c => c.type === 'budget-expectation');
  if (budgetExpectationConflicts.length > 2) {
    ...
    risks.factors.push({
      ...
      impact: { en: 'May need to compromise on finishes', fr: 'Peut nécessiter de faire des compromis sur les finitions' }
    });
  }
...
function generateSummary(analysis) {
  ...
  return {
    ...
    criticalIssues: analysis.conflicts.filter(c => c.severity === 'critical').length +
...
function estimateResolutionImpact(conflict, constraints) {
  ...
  return impactMapping[conflict.type] || { budget: 'variable', quality: 'variable', timeline: 'variable' };
}
...
function checkFeatureFeasibility(feature, constraints) {
  ...
  return {
    ...
    recommendation: blockers.length > 0
      ? { en: 'Feature not recommended due to hard constraints', fr: 'Fonctionnalité non recommandée en raison de contraintes strictes' }
      : warnings.length > 0
        ? { en: 'Feature possible with some compromises', fr: 'Fonctionnalité possible avec quelques compromis' }
