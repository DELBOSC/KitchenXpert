...
const STYLE_DEFINITIONS = {
  modern: {
    ...
    characteristics: ['clean-lines', 'minimal-ornamentation', 'flat-panel', 'integrated-handles', 'geometric-forms'],
  ...
  traditional: {
    ...
    characteristics: ['ornate-details', 'raised-panel', 'crown-molding', 'furniture-look', 'symmetry'],
  ...
  farmhouse: {
    ...
    characteristics: ['rustic-charm', 'open-shelving', 'apron-sink', 'natural-textures', 'vintage-elements'],
  ...
  contemporary: {
    ...
    characteristics: ['current-trends', 'mixed-materials', 'bold-elements', 'asymmetry', 'artistic-flair'],
  ...
  industrial: {
    ...
    characteristics: ['exposed-elements', 'raw-materials', 'metal-accents', 'utilitarian', 'urban-loft'],
  ...
  mediterranean: {
    ...
    characteristics: ['warm-tones', 'textured-walls', 'arched-elements', 'ornate-details', 'old-world-charm'],
...
const MATERIAL_COMPATIBILITY = {
  ...
  granite: {
    styles: { traditional: 95, transitional: 85, farmhouse: 75, mediterranean: 80, contemporary: 60 },
    priceRange: 'mid',
    durability: 90,
    maintenance: 'medium'
  },
...
const COLOR_RELATIONSHIPS = {
  complementary: [
    ['white', 'black'], ['gray', 'wood-tones'], ['cream', 'deep-blue'],
    ['sage', 'terracotta'], ['soft-blues', 'warm-cream']
  ],
  analogous: [
    ['white', 'cream', 'warm-neutrals'], ['gray', 'soft-gray', 'greige'],
    ['wood-tones', 'natural-wood', 'light-wood'], ['sage', 'olive', 'soft-blues']
  ],
  monochromatic: [
    ['white', 'cream', 'soft-colors'], ['gray', 'soft-gray', 'black'],
...
function analyzeStyle(responses) {
  ...
  analysis.coherenceScore = calculateStyleCoherence(aestheticAnswers, analysis.primaryStyle, analysis);

  // Determine confidence level
  analysis.confidenceLevel = determineConfidenceLevel(analysis.primaryStyle.confidence);

  // Identify and analyze conflicts
  analysis.conflicts = identifyStyleConflicts(aestheticAnswers, analysis.primaryStyle, budgetAnswers);

  // Calculate price impact
  analysis.priceImpact = calculatePriceImpact(analysis.primaryStyle, analysis.materialRecommendations);
...
function determinePrimaryStyle(aestheticAnswers, lifestyleAnswers) {
  ...
  const sortedScores = Object.entries(styleScores)
  ...
  selected.forEach(selectedColor => {
    COLOR_RELATIONSHIPS.complementary.forEach(pair => {
      if (pair.includes(selectedColor)) {
        ...
        if (complement && !selected.includes(complement) && !suggestions.find(s => s.color === complement)) {
          suggestions.push({
            color: complement,
            priority: 'medium',
            reason: {
...
function getHardwareDescription(hardware) {
  const descriptions = {
    'hidden': { en: 'Push-to-open or integrated handles for seamless look', fr: 'Ouverture push ou poignées intégrées pour un look épuré' },
    'integrated': { en: 'Built into cabinet design for clean lines', fr: 'Intégrées dans le design pour des lignes épurées' },
    'bar-pulls': { en: 'Sleek bar-style handles for modern appeal', fr: 'Poignées barres élégantes pour un look moderne' },
    'j-pulls': { en: 'Minimalist J-shaped edge pulls', fr: 'Tirettes J minimalistes en bordure' },
    'knobs': { en: 'Classic round knobs for timeless look', fr: 'Boutons ronds classiques pour un look intemporel' },
    'cup-pulls': { en: 'Bin-style cup pulls for vintage charm', fr: 'Tirettes coquille pour un charme vintage' },
    'ornate-handles': { en: 'Decorative handles for traditional elegance', fr: 'Poignées décoratives pour une élégance traditionnelle' },
    'antique-brass': { en: 'Antique brass finish for warmth', fr: 'Finition laiton antique pour la chaleur' },
    'bin-pulls': { en: 'Traditional bin-style for classic kitchens', fr: 'Style bac traditionnel pour cuisines classiques' },
    'black-iron': { en: 'Black iron hardware for farmhouse style', fr: 'Quincaillerie fer noir style ferme' },
    'matte-black': { en: 'Matte black finish for bold contrast', fr: 'Finition noir mat pour un contraste audacieux' },
    'simple-knobs': { en: 'Clean, simple knobs for versatility', fr: 'Boutons simples pour la polyvalence' },
    'statement-pieces': { en: 'Bold artistic hardware as focal points', fr: 'Quincaillerie artistique comme points focaux' },
    'mixed-finishes': { en: 'Combined finishes for visual interest', fr: 'Finitions combinées pour intérêt visuel' },
    'sculptural': { en: 'Sculptural artistic pieces', fr: 'Pièces sculpturales artistiques' },
    'leather-wrapped': { en: 'Leather-wrapped pulls for texture', fr: 'Tirettes gainées de cuir pour la texture' },
    'pipe-pulls': { en: 'Industrial pipe-style for loft look', fr: 'Style tuyau industriel pour look loft' },
    'metal-bars': { en: 'Raw metal bars for industrial edge', fr: 'Barres métal brut pour style industriel' },
    'exposed-hinges': { en: 'Visible hinges for utilitarian aesthetic', fr: 'Charnières visibles pour esthétique utilitaire' },
    'cast-iron': { en: 'Cast iron for rustic durability', fr: 'Fonte pour durabilité rustique' },
    'leather-pulls': { en: 'Leather pulls for Scandinavian warmth', fr: 'Tirettes cuir pour chaleur scandinave' },
    'wooden-knobs': { en: 'Wooden knobs for natural touch', fr: 'Boutons bois pour touche naturelle' },
    'simple-bar': { en: 'Simple bar pulls for clean design', fr: 'Barres simples pour design épuré' },
    'wrought-iron': { en: 'Wrought iron for Mediterranean charm', fr: 'Fer forgé pour charme méditerranéen' },
    'bronze': { en: 'Bronze finish for warm elegance', fr: 'Finition bronze pour élégance chaleureuse' },
    'ceramic-knobs': { en: 'Hand-painted ceramic for artisan touch', fr: 'Céramique peinte pour touche artisanale' },
    'antique-finish': { en: 'Antique finish for old-world charm', fr: 'Finition antique pour charme ancien' }
  };
...
function getLightingDescription(lightType) {
  const descriptions = {
    ...
    'linear-led': { en: 'Modern LED strips for task areas', fr: 'Bandes LED modernes pour zones de travail' },
    'geometric-pendant': { en: 'Angular pendant lights over island', fr: 'Suspensions géométriques au-dessus de l\'îlot' },
    'chandelier': { en: 'Elegant chandelier for focal point', fr: 'Lustre élégant comme point focal' },
    'lantern': { en: 'Lantern-style fixtures for warmth', fr: 'Lanternes pour une ambiance chaleureuse' },
    'pendant': { en: 'Pendant lights for island illumination', fr: 'Suspensions pour éclairer l\'îlot' }
  };

  return descriptions[lightType] || { en: formatColorName(lightType), fr: formatColorName(lightType) };
}
...
function generateBacksplashRecommendations(primaryStyle, materialRecs) {
  ...
  return styleDef.backsplash.map(backsplashType => ({
    type: backsplashType,
    style: primaryStyle.style,
    details: backsplashDetails[backsplashType] || { grout: 'visible', pattern: 'standard', maintenance: 'medium' },
...
function getBacksplashDescription(backsplashType) {
  const descriptions = {
    'subway-tile': { en: 'Classic subway tile in various layouts', fr: 'Carrelage métro classique en diverses poses' },
    'large-format-tile': { en: 'Large tiles for minimal grout lines', fr: 'Grands carreaux pour joints minimaux' },
    'glass': { en: 'Sleek glass panels for modern look', fr: 'Panneaux de verre pour look moderne' },
    'natural-stone': { en: 'Natural stone for organic beauty', fr: 'Pierre naturelle pour beauté organique' },
    'brick': { en: 'Exposed brick for rustic character', fr: 'Brique apparente pour caractère rustique' }
  };

  return descriptions[backsplashType] || { en: formatColorName(backsplashType), fr: formatColorName(backsplashType) };
}
...
function calculateStyleCoherence(aestheticAnswers, primaryStyle, analysis) {
  ...
  const cabinetStyle = aestheticAnswers['cabinet-style'];
  if (cabinetStyle) {
    const cabinetMatch = styleDef.characteristics.some(c =>
      c.includes(cabinetStyle) || cabinetStyle.includes(c.split('-')[0])
    );
...
function calculateMaintenanceProfile(analysis) {
  ...
  const tips = {
    low: [
      { en: 'Wipe surfaces regularly with mild cleaner', fr: 'Essuyez les surfaces régulièrement avec un nettoyant doux' },
      { en: 'Clean spills promptly to prevent staining', fr: 'Nettoyez les déversements rapidement pour éviter les taches' }
    ],
    medium: [
      { en: 'Seal natural stone annually', fr: 'Scellez la pierre naturelle annuellement' },
      { en: 'Use appropriate cleaners for each surface', fr: 'Utilisez des nettoyants appropriés pour chaque surface' }
    ],
    high: [
      { en: 'Regular professional maintenance recommended', fr: 'Entretien professionnel régulier recommandé' },
      { en: 'Avoid acidic substances on sensitive surfaces', fr: 'Évitez les substances acides sur les surfaces sensibles' }
    ]
  };
