...
function calculateLayoutOptions(dimensions, spatialAnswers, responses) {
  ...
  Object.entries(LAYOUT_REQUIREMENTS).forEach(([layout, requirements]) => {
    ...
    const combinedScore = Math.round(feasibility.score * 0.6 + functionalFit.score * 0.4);
...
function getLayoutPros(layout) {
  const pros = {
    ...
    peninsula: [
      {
        en: 'Island benefits, less space needed',
        fr: "Avantages de l'îlot, moins d'espace nécessaire",
      },
...
function calculateIslandOptions(dimensions, spatialAnswers) {
  ...
  if (preference === 'large-island' && maxIslandWidth >= 4) {
    ...
    options.features = [
      ...
      {
        id: 'seating',
        count: options.seatingCapacity,
        description: {
          en: `Seating for ${options.seatingCapacity}`,
          fr: `Places assises pour ${options.seatingCapacity}`,
        },
      },
      {
        id: 'storage',
        description: { en: 'Base cabinet storage', fr: 'Rangement dans les armoires de base' },
      },
      {
        id: 'cooktop-optional',
        description: { en: 'Optional cooktop', fr: 'Plaque de cuisson optionnelle' },
      },
    ...
    options.configurations.push({
      type: 'cooking-island',
      description: {
        en: 'Island with cooktop and prep sink',
        fr: 'Îlot avec plaque de cuisson et évier de préparation',
      },
    });
    options.configurations.push({
      type: 'prep-island',
      description: {
        en: 'Prep island with seating overhang',
        fr: 'Îlot de préparation avec débord pour sièges',
      },
    });
    ...
    options.features = [
      {
        id: 'prep-surface',
        description: { en: 'Additional prep surface', fr: 'Surface de préparation supplémentaire' },
      },
      {
        id: 'storage',
        description: { en: 'Under-counter storage', fr: 'Rangement sous le comptoir' },
      },
    ];
    if (options.seatingCapacity >= 2) {
      options.features.push({
        id: 'seating',
        count: 2,
        description: { en: 'Seating for 2', fr: 'Places assises pour 2' },
      });
    }
    ...
    options.features = [
      {
        id: 'flexible',
        description: { en: 'Moveable work surface', fr: 'Surface de travail mobile' },
      },
...
function generateIslandAlternatives(dimensions) {
  ...
  if (dimensions.width >= 8) {
    alternatives.push({
      type: 'peninsula',
      description: {
        en: 'Peninsula attached to existing counter',
        fr: 'Péninsule attachée au comptoir existant',
      },
    });
  }
...
function getTriangleRating(score) {
  if (score >= 85)
    return {
      level: 'optimal',
      description: { en: 'Optimal work triangle', fr: 'Triangle de travail optimal' },
    };
  if (score >= 70)
    return {
      level: 'good',
      description: { en: 'Good work triangle', fr: 'Bon triangle de travail' },
    };
  if (score >= 55)
    return {
      level: 'acceptable',
      description: { en: 'Acceptable work triangle', fr: 'Triangle de travail acceptable' },
    };
  return {
    level: 'poor',
    description: {
      en: 'Work triangle needs improvement',
      fr: 'Le triangle de travail nécessite des améliorations',
    },
  };
}
...
function analyzeClearances(dimensions, layout) {
  ...
  if (layout.layout === 'island') {
    ...
    analysis.islandClearance.adequate =
      analysis.islandClearance.available >= analysis.islandClearance.required;
  }

  // Overall clearance score
  const scores = [analysis.workAisle.adequate ? 1 : 0.5, analysis.walkway.adequate ? 1 : 0.5];
...
function assessErgonomics(userAnswers, spatialAnswers) {
  ...
  if (specialNeeds.includes('wheelchair-access')) {
    assessment.accessibilityNeeds.push({
      ...
      requirements: [
        ...
        {
          en: '30" knee clearance under sink',
          fr: '30" de dégagement pour les genoux sous l\'évier',
        },
      ],
    });
  ...
  if (specialNeeds.includes('height-considerations')) {
    assessment.heightConsiderations.push({
      ...
      requirements: [
        {
          en: 'Consider adjustable-height work surfaces',
          fr: 'Envisagez des surfaces de travail à hauteur réglable',
        },
...
function generateSpatialRecommendations(analysis, responses) {
  ...
  if (
    analysis.workTriangle?.rating?.level === 'poor' ||
    analysis.workTriangle?.rating?.level === 'acceptable'
  ) {
    recommendations.push({
      id: 'improve-triangle',
      type: 'workflow',
