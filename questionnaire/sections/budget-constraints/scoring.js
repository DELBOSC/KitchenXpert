...
const BUDGET_TIER_CONFIGURATIONS = {
  'under-10k': {
    ...
    warnings: [
      { en: 'Limited scope for structural changes', fr: 'Portée limitée pour les changements structurels' },
      { en: 'Consider phased approach for larger goals', fr: 'Envisagez une approche par phases pour des objectifs plus importants' }
    ]
  },
  '10k-25k': {
    score: 40,
    ...
    warnings: [
      { en: 'Choose priorities carefully - cannot address everything', fr: 'Choisissez vos priorités avec soin - ne peut pas tout aborder' }
    ]
  },
  '25k-50k': {
    score: 60,
    ...
    recommendations: {
      focus: ['full-cabinet-replacement', 'quality-countertops', 'appliance-package', 'professional-labor'],
  ...
  '50k-75k': {
    ...
    recommendations: {
      focus: ['quality-cabinets', 'premium-countertops', 'professional-appliances', 'custom-lighting'],
...
const FINANCING_CONFIGURATIONS = {
  'cash-savings': {
    ...
    advantages: {
      ...
      fr: ['Aucun frais d\'intérêt', 'Pouvoir de négociation maximum', 'Démarrage immédiat possible', 'Aucune dette']
    },
    considerations: {
  ...
  'home-equity': {
    score: 85,
    type: 'secured-loan',
    interestRate: { typical: 0.07, range: { min: 0.05, max: 0.10 } },
    advantages: {
      en: ['Lower interest rates', 'Tax-deductible interest (consult tax advisor)', 'Larger amounts available'],
      fr: ['Taux d\'intérêt plus bas', 'Intérêts déductibles d\'impôt (consultez un conseiller)', 'Montants plus importants disponibles']
    },
    considerations: {
      en: ['Home used as collateral', 'Requires equity and approval', 'Closing costs apply'],
      fr: ['Maison utilisée comme garantie', 'Nécessite de l\'équité et une approbation', 'Frais de clôture applicables']
    },
  ...
  'personal-loan': {
    score: 70,
    type: 'unsecured-loan',
    interestRate: { typical: 0.10, range: { min: 0.07, max: 0.15 } },
    advantages: {
    ...
    considerations: {
      ...
      fr: ['Taux d\'intérêt plus élevés', 'Impact sur la cote de crédit', 'Généralement des montants plus petits']
    },
  ...
  'credit-card': {
    score: 45,
    type: 'revolving',
    interestRate: { typical: 0.20, range: { min: 0.15, max: 0.25 } },
    advantages: {
    ...
    considerations: {
      en: ['Very high interest', 'Should only use for small items', 'Requires immediate payoff plan'],
      fr: ['Intérêt très élevé', 'À utiliser uniquement pour petits articles', 'Nécessite un plan de remboursement immédiat']
    },
    tags: ['small-purchases-only', 'rewards-possible'],
    effectiveBudgetMultiplier: 0.85,
    timelineImpact: 'none',
    warnings: [
      { en: 'Credit cards should only be used for small purchases with immediate payoff plan', fr: 'Les cartes de crédit ne devraient être utilisées que pour de petits achats avec un plan de remboursement immédiat' }
    ]
  },
  'contractor-financing': {
    ...
    considerations: {
      ...
      fr: ['Comparez les taux attentivement', 'Peut limiter le choix des entrepreneurs', 'Lisez les conditions attentivement']
    },
  ...
  'phased': {
    ...
    advantages: {
      ...
      fr: ['Payez au fur et à mesure', 'Aucun intérêt si vous épargnez entre les phases', 'Flexibilité pour ajuster la portée']
    },
    considerations: {
      en: ['Extended timeline', 'Living in construction zone', 'Price increases over time'],
      fr: ['Délai prolongé', 'Vivre dans une zone de construction', 'Augmentation des prix au fil du temps']
    },
...
const PRIORITY_SPENDING_CONFIGURATIONS = {
  ...
  'countertops': {
    typicalAllocation: { min: 0.10, max: 0.20 },
  ...
  'layout': {
    typicalAllocation: { min: 0.10, max: 0.25 },
    importance: 'high',
    description: {
...
const SAVINGS_AREAS_CONFIGURATIONS = {
  ...
  'keep-layout': {
    savingsPercent: { min: 0.10, max: 0.25 },
    description: {
  ...
  'refurbish-cabinets': {
    savingsPercent: { min: 0.15, max: 0.30 },
    description: {
  ...
  'mix-stock-custom': {
    savingsPercent: { min: 0.10, max: 0.20 },
    description: {
  ...
  'phase-project': {
    savingsPercent: { min: 0.05, max: 0.10 },
    description: {
...
const CONTINGENCY_CONFIGURATIONS = {
  'yes-20-plus': {
    score: 100,
    level: 'well-prepared',
    percentageRange: { min: 0.20, max: 0.30 },
    description: {
  ...
  'yes-10-20': {
    score: 80,
    level: 'prepared',
    percentageRange: { min: 0.10, max: 0.20 },
    description: {
  ...
  'some-buffer': {
    ...
    warnings: [
      { en: 'Recommend building at least 10% contingency', fr: 'Recommandons de constituer au moins 10% de contingence' }
    ]
  },
  'no-buffer': {
    score: 20,
    ...
    warnings: [
      { en: 'Critical: Kitchen renovations commonly have unexpected costs. Consider reducing scope.', fr: 'Critique: Les rénovations de cuisine ont souvent des coûts imprévus. Envisagez de réduire la portée.' }
    ]
  }
};
...
function scoreBudgetFlexibility(value) {
  ...
  const levelDescriptions = {
    en: ['Very strict, no room for changes', 'Slight flexibility for essentials', 'Moderate flexibility', 'Quite flexible for upgrades', 'Very flexible, quality is priority'],
    fr: ['Très strict, pas de marge', 'Légère flexibilité pour l\'essentiel', 'Flexibilité modérée', 'Assez flexible pour les améliorations', 'Très flexible, la qualité est prioritaire']
  };
...
function scoreSavingsAreas(values) {
  ...
  totalSavingsPercent = Math.min(0.30, totalSavingsPercent);
...
function getFlexibilityLevel(value) {
  ...
  return ['strict', 'slightly-flexible', 'moderate', 'quite-flexible', 'very-flexible'][level - 1] || 'moderate';
}
...
function calculatePreparednessScore(answers) {
  ...
  score += flexImpact;
  factors.push({ factor: 'flexibility', impact: flexImpact, level: flex >= 4 ? 'good' : flex <= 2 ? 'limited' : 'moderate' });
...
function calculateInvestmentMindset(answers) {
  ...
  return {
    score,
    focus: answers['roi-consideration'] === 'very-important' ? 'resale' :
           answers['roi-consideration'] === 'not-important' ? 'personal' : 'balanced',
    considerations,
...
function calculateFinancialHealth(answers, componentScores) {
  ...
  healthScore += financingScore * 0.30;
  factors.push({
  ...
  healthScore += flexibilityScore * 0.20;
  factors.push({
...
function calculatePriorityAnalysis(answers, budgetTier) {
  ...
  const allocationTemplate = BUDGET_ALLOCATION_TEMPLATES[budgetTier] || BUDGET_ALLOCATION_TEMPLATES.mid;
  ...
  if (priorities.includes('appliances') && priorities.includes('cabinets') && priorities.includes('countertops')) {
...
function generateRecommendations(answers, componentScores, scores) {
  ...
  if ((budgetTier === 'budget' || budgetTier === 'entry') && answers['financing-method'] !== 'phased') {
